...
func _get_moves_for_level(level_number: int) -> int:
	if level_number <= 10:
		return 24 - int(floor(float(level_number - 1) / 4.0))
	if level_number <= 30:
		return 23 - int(floor(float(level_number - 11) / 7.0))
	if level_number <= 60:
		return 22 - int(floor(float(level_number - 31) / 10.0))
	return 21 - int(floor(float(level_number - 61) / 14.0))


func _get_reward_for_level(level_number: int) -> int:
	if level_number <= 10:
		return 1 + int(floor(float(level_number - 1) / 4.0))
	if level_number <= 30:
		return 2 + int(floor(float(level_number - 11) / 10.0))
	if level_number <= 60:
		return 3 + int(floor(float(level_number - 31) / 15.0))
	return 4 + int(floor(float(level_number - 61) / 20.0))
