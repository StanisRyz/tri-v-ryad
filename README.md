...
- A playable battle screen with a top-to-bottom order of `EnemyPanel`, then `RoundModifierPanel`, then the widened 9x9 `BoardView`, then a compact `BottomHudRow` (Level/Moves/Levels) and the booster panel near the bottom (Stage 64.4; Stage 64.1 originally put the HUD row at the top), placeholder `TileView` tiles, hidden inactive hero party panel, hidden status text, result overlay, and clean Next Level, Retry, and Levels result actions.
- Reusable UI components: `BattleHud`, `EnemyPanel`, `HeroPartyPanel`, `HeroCard`, `BattleResultOverlay`, and `ImageSlot`.
- `EnemyPanel` (Stage 64.1) is now an asset-based visual panel: a `BackgroundVisual`/`EnemySprite` pair of `FallbackImageSlot`s, a red `HpBarFill`/`HpBarBackground` bar sized by `current_hp / max_hp`, and an `HpValueLabel` showing `"current / max"` with no `"HP:"` prefix. The old `EnemyNameLabel`/`EnemyHpLabel`/`EnemyIntentLabel`/`EnemyTargetLabel`/small `EnemyImageSlot` text row is gone; `set_enemy_state()` still exists for compatibility but ignores intent/target. `set_enemy_textures()`/`play_damage_feedback()` support normal/damaged enemy sprite states (currently only populated for `gatekeeper`), falling back to the existing flash/shake feedback when no damaged texture is available.
- `RoundModifierPanel` (Stage 64.1) dropped `ModifierNameLabel`; it now shows only `ModifierDescriptionLabel` for the active level boost's description.
...
## Stage 62.4: Shop Catalog and Purchase Resolver v0.1

Stage 62.4 is complete. It adds the backend foundation for the shop — item categories, purchase kinds, structured reward data, a catalog of concrete items, and a purchase resolver that spends currency through `ProgressManager` — without touching the visible shop UI, which stays the Stage 61 `ShopPlaceholderScreen`. Real shop tabs, item cards, and buy buttons are planned for Stage 62.5.

- **New `scripts/game/shop/` folder** holding all Stage 62.4 backend classes, mirroring the `progression`/`config` folder conventions (plain `RefCounted` classes, no autoloads).
- **`ShopItemCategory`** (`shop_item_category.gd`). Stable string category IDs: `"boosters"`, `"gems"`, `"bundles"`, plus `ALL_IDS`/`is_valid()`.
- **`ShopPurchaseKind`** (`shop_purchase_kind.gd`). Two purchase kinds: `"currency"` (buyable now through `ProgressManager` gold/gems) and `"external_payment"` (listed but not yet purchasable — no real payment SDK is connected).
- **`ShopRewardType`** (`shop_reward_type.gd`). Structured reward dictionaries with `make_currency_reward(currency_id, amount)`/`make_booster_reward(booster_id, amount)` builders and `is_valid()`, matching the `{"type": "currency", "currency_id": ..., "amount": ...}` / `{"type": "booster", "booster_id": ..., "amount": ...}` shapes used by the Stage 62.3 `LevelStarRewardResolver`.
- **`ShopItemConfig`** (`shop_item_config.gd`). Fields: `item_id`, `category`, `display_name`, `description`, `purchase_kind`, `price_currency_id`, `price_amount`, `rewards`, `is_available`, plus `is_valid()` (currency items require a valid price currency/amount; every item requires at least one valid reward).
- **`ShopCatalog`** (`shop_catalog.gd`). Registers all Stage 62.4 items on construction and exposes `get_all_items()`, `get_items_by_category(category)`, `get_item(item_id)`, `has_item(item_id)`. Registers:
  - 6 booster purchases (currency): `booster_hammer_gold`/`booster_freeze_time_gold`/`booster_rocket_barrage_gold` at 20 gold each, and `booster_hammer_gems`/`booster_freeze_time_gems`/`booster_rocket_barrage_gems` at 10 gems each — each grants +1 of the matching `BoosterCatalog` booster.
  - 4 gem products (external_payment placeholders): `gems_50`/`gems_150`/`gems_250`/`gems_500` granting 50/150/250/500 gems.
  - 4 bundles (external_payment placeholders): `bundle_small` (50 gems, 50 gold, +1 of each booster), `bundle_medium` (100 gems, 100 gold, +2 of each booster), `bundle_large` (200 gems, 200 gold, +3 of each booster), `bundle_mega` (500 gems, 500 gold, +10 of each booster).
- **`ShopPurchaseResolver`** (`shop_purchase_resolver.gd`). `purchase(item_id, progress_manager, shop_catalog) -> Dictionary` returns `{accepted, reason, item_id, spent, granted}`. For `currency` items it validates the item/progress/price/rewards, checks `ProgressManager.can_spend_currency()`, spends the price via `ProgressManager.spend_currency()`, then applies every reward via `ProgressManager.add_currency()`/`add_booster()` (recording each in `spent`/`granted`), returning `accepted = true` and reason `""`. For `external_payment` items it never touches progress and always returns `accepted = false`, reason `"payment_not_connected"`. Other reasons: `invalid_item`, `not_enough_gold`, `not_enough_gems`, `invalid_progress`, `invalid_price`, `invalid_reward`.
- **`ShopItemFormatter`** (`shop_item_formatter.gd`). `format_item_title()`, `format_item_description()`, `format_price()` (e.g. `"Price: 20 Gold"`, or `"Price: not available"` for external-payment items), and `format_rewards()` (e.g. `"Gives: +1 Hammer"`, `"Gives: 50 Gems, 50 Gold, +1 Hammer, +1 Time Freeze, +1 Rocket Barrage"`), reading booster display names from `BoosterCatalog` rather than hardcoding them.
- **`ShopPurchaseFormatter`** (`shop_purchase_formatter.gd`). `format_purchase_result(result: Dictionary) -> String` maps a resolver result to player-facing text: `"Purchased!"`, `"Not enough gold"`, `"Not enough gems"`, `"Payments are not connected yet"`, `"Item unavailable"`, or a generic `"Purchase failed"` fallback for the remaining invalid-state reasons.
- **`ShopPlaceholderScreen` unchanged in this stage.** No shop tabs, item cards, buy buttons, icons, animations, or payment SDK were added — the visible shop screen was still the Stage 61 placeholder at the end of Stage 62.4. Stage 62.5 (below) builds the real UI on top of this backend.
- **Unchanged by design.** Star milestone rewards, global booster spending in battle, wallet/booster inventory storage, `MainMenu` navigation, deterministic layouts/boosts, fixed enemy HP/move curve, holes, ice, no-move shuffle, and the victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected.

## Stage 62.5: Shop UI Tabs and Purchase Screens v0.1

Stage 62.5 is complete. It replaces the Stage 61 `ShopPlaceholderScreen` with a real `ShopScreen` built entirely on the Stage 62.4 backend (`ShopCatalog`, `ShopPurchaseResolver`, `ShopItemFormatter`, `ShopPurchaseFormatter`) — no prices, rewards, or messages are hardcoded in the UI layer. Yandex payments, real-money SDK, ads, and receipt/server validation remain out of scope and are planned for a later stage.

- **New `ShopScreen`** (`scripts/screens/shop_screen.gd`, `scenes/screens/ShopScreen.tscn`), following the same `Control` root / `SafeMargin` (`MarginContainer`) / `Root` (`VBoxContainer`) layout convention as `LevelSelectScreen`. Shows title "Магазин", a wallet row (`%GoldLabel`/`%GemsLabel`, "Gold: X"/"Gems: Y"), a tab row (`%BoostersTabButton`/`%GemsTabButton`/`%BundlesTabButton` — "Бустеры"/"Гемы"/"Пакеты"), a `ScrollContainer` wrapping a 2-column `%ItemGrid` (`GridContainer`) of item cards, a `%FeedbackLabel` for purchase messages, and a `%BackButton` ("Назад").
- **New `ShopItemCard`** (`scripts/ui/shop_item_card.gd`, `scenes/ui/ShopItemCard.tscn`), a small reusable `PanelContainer` card with title/description/rewards/price `Label`s and a `%BuyButton` ("Купить"). `set_item(item)` populates every label purely via `ShopItemFormatter.format_item_title()`/`format_item_description()`/`format_price()`/`format_rewards()` — the card never reads `item.price_amount`/`item.rewards` directly. Emits `purchase_pressed(item_id)` when bought, mirroring how `BoosterButton`'s native `pressed` signal bubbles through `BoosterPanel` via `.bind()`.
- **Catalog-driven, no hardcoding.** `ShopScreen._refresh_items()` clears `%ItemGrid`'s children and rebuilds them from `ShopCatalog.get_items_by_category(_selected_category)` — the screen owns a `ShopCatalog` instance and never hardcodes item IDs, prices, or reward text; `ShopItemCategory.BOOSTERS`/`GEMS`/`BUNDLES` map directly to the three tabs.
- **Tab switching.** `_show_category(category)` sets `_selected_category`, dims the two inactive tab buttons via `modulate` (matching the existing UI convention of state-driven visual feedback rather than a `TabContainer`, since the project had no prior tab pattern to reuse), and rebuilds the item grid. The screen opens on the Бустеры tab by default.
- **Purchase flow.** `ShopItemCard.purchase_pressed` is connected per-card in `_refresh_items()`; `_on_item_purchase_pressed(item_id)` calls `ShopPurchaseResolver.purchase(item_id, _progress_manager, _shop_catalog)`, sets `%FeedbackLabel.text` from `ShopPurchaseFormatter.format_purchase_result(result)`, always refreshes the wallet labels, and rebuilds the item grid only when `result.accepted` is true (an unavailable/insufficient-funds/payment-not-connected attempt never touches progress, so nothing needs to be redrawn on cards themselves this stage).
- **Booster purchases work end-to-end.** Buying `booster_hammer_gold`/`booster_freeze_time_gold`/`booster_rocket_barrage_gold` (20 gold each) or the gem variants (10 gems each) spends the correct currency via `ProgressManager.spend_currency()`, grants +1 of the matching booster via `ProgressManager.add_booster()`, refreshes the wallet labels, and shows "Purchased!" — verified with a headless scripted run (isolated scratch save, never touching the real player save) that pressed a booster card's Купить button and confirmed gold dropped by 20 and the hammer inventory count increased by 1; the battle `BoosterPanel` reads the same `ProgressManager.get_booster_count()` used since Stage 62.2, so the new inventory is visible in battle with no extra wiring.
- **Gem products and bundles remain payment placeholders.** All 4 gem cards and all 4 bundle cards are `purchase_kind = external_payment`; pressing Купить on any of them calls the same resolver, which returns `accepted = false`/reason `payment_not_connected` without touching `progress`, and `%FeedbackLabel` shows "Payments are not connected yet" — verified in the same headless run (wallet labels unchanged after the attempt). Bundle reward text (e.g. "Gives: 50 Gems, 50 Gold, +1 Hammer, +1 Time Freeze, +1 Rocket Barrage") is fully visible on the card via `ShopItemFormatter.format_rewards()`.
- **`App` navigation rewired.** `SHOP_PLACEHOLDER_SCREEN` was replaced with `SHOP_SCREEN := preload("res://scenes/screens/ShopScreen.tscn")`; `_show_shop_placeholder()` became `_show_shop_screen()`, which now injects `_progress_manager` via `set_progress_manager()`/`refresh_progress_state()` exactly like `_show_level_select()` does, before connecting `back_pressed` to a renamed `_on_shop_back_pressed()` that returns to MainMenu. `_on_main_menu_shop_pressed()` now calls `_show_shop_screen()`. The old `ShopPlaceholderScreen` scene/script files are left in place but are no longer referenced from `App` — nothing else in the project pointed at them besides the pre-existing `direct_startup_flow_test.gd`, which was not touched per this stage's explicit test-freeze scope and will need updating separately.
- **Unchanged by design.** MainMenu, Settings, LevelSelect, GameScreen, star milestone rewards, global booster spending, wallet/booster inventory save data, deterministic layouts/boosts, fixed enemy HP/move curve, holes, ice, no-move shuffle, and victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected; a headless scripted smoke pass (isolated scratch save path, deleted after use) was used only to sanity-check the new screen during development and is not part of the patch.

## Stage 63.1: Asset-Based Main Menu UI v0.1

Stage 63.1 replaces the technical `MainMenu` (a `CenterContainer`/`VBoxContainer` of plain `Button`s) with an asset-based presentation: a static background image and separate default/pressed texture buttons. Navigation signals, `App` routing, and every gameplay/economy system are unchanged — this stage is presentation-only.

- **New reusable `PressableTextureButton`** (`scripts/ui/pressable_texture_button.gd`, `class_name PressableTextureButton extends Button`). Exports `normal_texture`, `pressed_texture`, `press_duration := 0.2`, `button_text`, `placeholder_color`, `label_font_size`. Clears the default Button styleboxes/font colors so no native Godot chrome shows through, and builds three internal children on `_ready()`: a `Placeholder` `ColorRect` (visible only when `normal_texture` is `null`, so a missing texture renders as a flat color instead of a blank/broken button), a full-rect `Texture` `TextureRect`, and a full-rect `ButtonLabel` `Label` (centered, `mouse_filter = IGNORE`) carrying `button_text` over the (currently empty) art. On the native `pressed` signal: if an animation is already pending the press is ignored (no repeat-click stacking); otherwise it swaps to `pressed_texture`, nudges the label down by `LABEL_PRESS_OFFSET` (2-4px per the v0.1 polish spec), awaits `press_duration` seconds via `get_tree().create_timer()`, restores `normal_texture` and the label position, and only then emits `delayed_pressed` — the component's only new public signal. Reusable for any future image-button UI.
- **New main menu asset folders and registered asset keys**, following the existing `GameAssetCatalog`/`AssetKeyResolver` convention rather than raw `ext_resource` paths (so a missing PNG degrades safely instead of breaking scene load): `assets/images/ui/main_menu/background.png` and `assets/images/ui/main_menu/buttons/{play,level_select,shop,settings}_{default,pressed}.png`. `GameAssetCatalog.ASSET_MAP` gained `main_menu_background` and the 8 button keys; `AssetKeyResolver` gained `get_main_menu_background_asset_key()` and `get_main_menu_button_asset_key(button_id, state)`. None of the PNG files exist yet — `GameAssetCatalog.try_load_texture_cached()` returns `null` for a missing path (via `ResourceLoader.exists()`), so `PressableTextureButton` simply shows its placeholder color and the background `ImageSlot` shows its placeholder color; nothing crashes.
- **Rebuilt `MainMenuScreen`** (`scenes/screens/MainMenuScreen.tscn`, `scripts/screens/main_menu_screen.gd`). New structure: `Background` (`ImageSlot`, same reusable placeholder-on-missing-texture component `LevelSelectScreen`/`SettingsScreen` already use), `GoldLabel`/`GemsLabel` (top corners), `PlayButton`/`LevelSelectButton`/`ShopButton`/`SettingsButton` (`PressableTextureButton`, each ~430x164 — close to the requested 495:189 aspect ratio — manually positioned in the lower half of the 720x1280 portrait layout), and a `HeroesButton` (plain legacy `Button`, `visible = false`, gated by `FeatureFlags.HERO_SYSTEMS_ENABLED` exactly as before). `_ready()` binds the background and all four button textures from `GameAssetCatalog`/`AssetKeyResolver` and sets each button's Russian label ("Играть"/"Выбрать уровень"/"Магазин"/"Настройки").
- **Signals preserved exactly.** `play_pressed`, `level_select_pressed`, `shop_pressed`, `heroes_pressed`, `settings_pressed` all still exist with the same names/shapes. Each `PressableTextureButton.delayed_pressed` (not the raw `Button.pressed`) is what triggers the corresponding signal emit, so navigation only fires after the full 0.2s pressed-texture animation completes — `App` never changed its `_show_main_menu()` signal-connection code besides the new progress-manager wiring below.
- **Wallet labels wired via `ProgressManager`.** `main_menu_screen.gd` gained `set_progress_manager(progress_manager)` and `refresh_progress_state()`, matching the exact convention already used by `ShopScreen`/`LevelSelectScreen`. `_refresh_wallet_labels()` reads `ProgressManager.get_currency(CurrencyType.GOLD)`/`get_currency(CurrencyType.GEMS)` (falling back to `0`/`0` when no `ProgressManager` is attached) and sets `%GoldLabel.text`/`%GemsLabel.text` to `"Gold: %d"`/`"Gems: %d"`.
- **`App` injects `ProgressManager` into MainMenu.** `_show_main_menu()` now calls `screen.set_progress_manager(_progress_manager)` and `screen.refresh_progress_state()` before wiring signals — the same pattern already used for Shop/LevelSelect/GameScreen. Because `ScreenRouter.change_screen()` always instantiates a fresh `MainMenuScreen`, every return to MainMenu (from Shop, LevelSelect, Settings, or GameScreen's back-flow) already re-enters `_show_main_menu()` and re-injects the current wallet state — no extra refresh hooks were needed elsewhere.
- **Unchanged by design.** `PlayLevelResolver`, LevelSelect routing, `ShopScreen` tabs/purchases, `SettingsScreen`, `GameScreen` result flow, star milestone rewards, global booster spending, wallet/booster inventory save data, deterministic layouts/boosts, fixed enemy HP/move curve, holes, ice, no-move shuffle, and victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected; a headless `--editor --quit` import pass and a brief headless run were used only to confirm the new scene/scripts parse and initialize without errors, and are not part of the patch.

## Stage 63.1.1: Anchored Main Menu Labels v0.1

Stage 63.1.1 gives `PressableTextureButton` its own internal text layer so button textures stay art-only and button text is rendered by a real Godot `Label`, and wraps the MainMenu wallet labels in dedicated field zones — both anchored relative to their parent so text follows its button/field across resolution changes instead of living as free screen-level nodes.

- **`PressableTextureButton` owns a dedicated text zone.** `pressable_texture_button.gd` builds a full-rect `TextZone` (`MarginContainer`, `mouse_filter = IGNORE`) above the `Texture`/`Placeholder` layers, owning the `ButtonLabel` `Label` (centered, expand/fill) as its only child. New exports `text_margin_left`/`text_margin_top`/`text_margin_right`/`text_margin_bottom` (floats, applied as `MarginContainer` theme constant overrides) place the text inside the button without touching the art; `label_font_size` was renamed `text_font_size`; the previously-hardcoded label press-offset constant is now an exported `pressed_text_offset: Vector2` (default unchanged). `button_text` remains exported and editable in the Inspector.
- **Pressed animation unchanged in behavior.** On press: swap to `pressed_texture`, shift the whole `TextZone` by `pressed_text_offset`, await `press_duration` (0.2s), restore both, then emit `delayed_pressed`. Repeated clicks while the animation is pending are still ignored.
- **Text stays anchored to its button.** Because `TextZone`/`ButtonLabel` are children of the button, not screen-level nodes, moving or resizing a button in the Inspector automatically carries its text along; only the text zone's own margins need adjusting to fine-tune placement within the button.
- **MainMenuScreen wallet labels wrapped in field zones.** `GoldLabel`/`GemsLabel` are no longer direct children of `MainMenuScreen`; each now lives inside its own `GoldField`/`GemsField` `Control`, manually positioned over the background's currency panels, with the label full-rect anchored and vertically centered inside its field. Moving/resizing a field in the Inspector keeps its label attached the same way a button's text zone keeps text attached to the button.
- **Navigation and wallet refresh unchanged.** `play_pressed`/`level_select_pressed`/`shop_pressed`/`heroes_pressed`/`settings_pressed` and `App`'s MainMenu routing are byte-for-byte unchanged; `set_progress_manager()`/`refresh_progress_state()` still drive `%GoldLabel`/`%GemsLabel` via `ProgressManager.get_currency()`, falling back to 0/0 with no `ProgressManager` attached.
- **Unchanged by design.** `ShopScreen`, `LevelSelectScreen`, `GameScreen`, `SettingsScreen`, star milestone rewards, global booster spending, wallet/booster inventory save data, deterministic layouts/boosts, fixed enemy HP/move curve, holes, ice, no-move shuffle, and victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected.

## Stage 63.1.2: Editor-visible Main Menu Textures and Child Labels v0.1

Stage 63.1.2 makes the asset-based MainMenu fully editable in the Godot editor. Backgrounds and button textures are now visible when opening `MainMenuScreen.tscn`, not only at runtime, and `PressableTextureButton` owns explicit child nodes for its texture and text so both can be manually positioned in the Inspector.

- **`Background` is now an editor-visible `TextureRect`.** It replaces the runtime-only `ImageSlot`-scripted `ColorRect`: a plain `TextureRect`, full-rect anchored, with its `texture` assigned directly via `ExtResource` to `background.png` right in the scene file. The background renders immediately on opening the scene, with no script needing to run.
- **`PressableTextureButton` is now `@tool`.** `pressable_texture_button.gd` runs its normal `_ready()`/exported setters in the editor too, so `normal_texture` (and `button_text`) preview live without pressing Play; `pressed.connect(_on_pressed)` is skipped in the editor (`Engine.is_editor_hint()`) so no click handling is wired up outside the running game.
- **Explicit child nodes, declared in the scene.** Each button now has `ButtonTexture` (`TextureRect`, full-rect, `mouse_filter = IGNORE`), `TextMargin` (`MarginContainer`, full-rect, `mouse_filter = IGNORE`), and `TextMargin/Label` (centered, expand/fill, `mouse_filter = IGNORE`) written directly into `MainMenuScreen.tscn`, with `normal_texture`/`pressed_texture` assigned via `ExtResource` to the real button PNGs. The script looks these up (`get_node_or_null()`) before creating anything, so pre-declared scene children are reused rather than duplicated; a `Placeholder` `ColorRect` fallback is still auto-created (and pinned behind the art via `move_child(..., 0)`) if a texture is ever missing. Any node the script does have to create while running in the editor gets its `owner` set to the edited scene root, so a manual scene save persists it as a real node.
- **Label text is anchored inside the button.** `TextMargin/Label`'s text comes from `button_text`, font size from `text_font_size`, and position from `TextMargin`'s exported margins — all children of the button, so moving/resizing a button in the Inspector carries its text along automatically.
- **Gold/Gems field labels unchanged.** `GoldField`/`GemsField`/`GoldLabel`/`GemsLabel` keep the Stage 63.1.1 structure — labels are children of their field zones, manually positioned over the background's currency panels.
- **Pressed animation and signals preserved.** Runtime behavior is identical: press ignores repeats while pending, swaps to `pressed_texture`, shifts `TextMargin` by `pressed_text_offset`, waits `press_duration` (0.2s), restores both, then emits `delayed_pressed`; `play_pressed`/`level_select_pressed`/`shop_pressed`/`heroes_pressed`/`settings_pressed` and `App`'s MainMenu routing are unchanged.
- **Runtime texture binding no longer clobbers the editor-assigned art.** `main_menu_screen.gd`'s `_apply_background_texture()`/`_bind_button_texture()` only overwrite a texture when `GameAssetCatalog.try_load_texture_cached()` returns non-null, preserving the scene-assigned fallback and the existing `GameAssetCatalog`/`AssetKeyResolver` convention.
- **Unchanged by design.** `ShopScreen`, `LevelSelectScreen`, `GameScreen`, `SettingsScreen`, star milestone rewards, global booster spending, wallet/booster inventory save data, deterministic layouts/boosts, fixed enemy HP/move curve, holes, ice, no-move shuffle, and victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected.

## Stage 63.1.3: Main Menu Label Nodes Cleanup v0.1

Stage 63.1.3 fixes a node-collision bug from Stage 63.1.2 and simplifies the wallet labels so the whole MainMenu scene is easy to hand-tune in the Godot editor.

- **Root cause fixed.** Godot applies a node's exported properties before that node's own tscn-declared children are attached during scene instantiation. Stage 63.1.2's `_ensure_children()` ran from every property setter, so it could create a fresh `ButtonTexture`/`TextMargin` before the real scene-declared ones existed — when the real nodes attached afterward, Godot auto-renamed the collision to `ButtonTexture2`/`TextMargin2`, and `TextMargin/Label` was never reliably created. `pressable_texture_button.gd` now gates every `_update_*()` function behind a `_ready_done` flag (false until the end of `_ready()`), so a setter firing before `_ready()` is a safe no-op; `_ensure_children()` itself runs exactly once, from `_ready()`, after children are guaranteed attached.
- **Clean button child structure.** `MainMenuScreen.tscn`'s `PlayButton`/`LevelSelectButton`/`ShopButton`/`SettingsButton` each now have exactly one `ButtonTexture`/`TextMargin`/`TextMargin/Label` — no more `...2` duplicates — with `button_text` visible as a real white, black-outlined `Label` centered in the button.
- **Readable label styling.** `Label` carries white `font_color`, a black `font_outline_color` with `outline_size = 4` for contrast against bright button art, `font_size = 30`, and centered alignment; `pressable_texture_button.gd` applies the same styling to any node it creates at runtime, so this holds even for a fresh scene. Text position is still controlled purely by `TextMargin`'s exported `text_margin_left`/`top`/`right`/`bottom`.
- **Gold/Gems labels simplified.** `GoldField`/`GemsField` wrapper `Control`s were removed; `GoldLabel`/`GemsLabel` are now direct children of `MainMenuScreen` with plain, unnested offsets, so they can be dragged/resized directly over the background's currency panels. Both keep `unique_name_in_owner = true`, so `%GoldLabel`/`%GemsLabel` lookups in `main_menu_screen.gd` are unchanged.
- **Runtime asset binding is now true fallback-only.** `_apply_background_texture()` returns immediately if `Background.texture` is already assigned; `_bind_button_texture()` only calls `GameAssetCatalog.try_load_texture_cached()` for whichever of `normal_texture`/`pressed_texture` is still `null` on the button. An editor-assigned texture is never touched by the runtime rebind.
- **Pressed animation and navigation preserved.** Press still ignores repeats while pending, swaps to `pressed_texture`, shifts `TextMargin` by `pressed_text_offset`, waits `press_duration` (0.2s), restores both, then emits `delayed_pressed`; `play_pressed`/`level_select_pressed`/`shop_pressed`/`heroes_pressed`/`settings_pressed` and `App`'s MainMenu routing are unchanged.
- **Unchanged by design.** `ShopScreen`, `LevelSelectScreen`, `GameScreen`, `SettingsScreen`, star milestone rewards, global booster spending, wallet/booster inventory save data, deterministic layouts/boosts, fixed enemy HP/move curve, holes, ice, no-move shuffle, and victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected.

## Stage 63.1.4: Inspector-driven Main Menu Label Layout v0.1

Stage 63.1.4 fixes a follow-on bug from Stage 63.1.3: `PressableTextureButton` was silently discarding hand-tuned Label alignment/style changes made in the Inspector every time the game started, because `_ensure_children()` force-applied its default styling to the Label unconditionally, not just when creating one.

- **Default styling now applies only to a freshly created Label.** `pressable_texture_button.gd`'s `_ensure_children()` used to set `horizontal_alignment`/`vertical_alignment`/`size_flags_horizontal`/`size_flags_vertical`/`font_color`/`font_outline_color`/`outline_size` on `TextMargin/Label` every single time it ran, even when the Label already existed as a scene node. Those lines moved inside the `if _label == null:` branch, so they run exactly once — only when the script itself has to create a missing Label — and never again touch a Label that was found via `get_node_or_null()`.
- **Existing scene Labels keep Inspector settings as the source of truth.** `PlayButton/TextMargin/Label`, `LevelSelectButton/TextMargin/Label`, `ShopButton/TextMargin/Label`, and `SettingsButton/TextMargin/Label` can each be independently left/center/right-aligned (or otherwise restyled) in the Inspector, saved, and that setting survives every subsequent scene load and game run.
- **`button_text` still drives Label text.** `_update_label()` still sets `Label.text = button_text` on every call, and `_update_label_font_size()` still applies `text_font_size`, and `_update_text_margins()` still drives `TextMargin`'s margins from the exported `text_margin_left`/`top`/`right`/`bottom` — none of that changed, since alignment/color/outline/size-flags were the only source of the bug.
- **Pressed animation and navigation preserved.** `normal_texture`/`pressed_texture` editor/runtime preview, the 0.2s pressed-texture animation (`pressed_text_offset` shifting `TextMargin`, repeat clicks ignored while pending), and `delayed_pressed` firing only after the animation completes are all unchanged.
- **Unchanged by design.** `App`'s MainMenu routing, `ShopScreen`, `LevelSelectScreen`, `GameScreen`, `SettingsScreen`, economy, rewards, booster spending, deterministic layouts, holes, ice, and result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected.

## Stage 63.2: Settings UI Shell v0.1

Stage 63.2 rebuilds `SettingsScreen` from a technical `VBoxContainer`/plain-`Button` layout into an asset-based settings UI shell: a shared background (reusable by future menu-like screens), a centered ~620x620 settings window, the user-facing settings inside that window, and an asset-based `PressableTextureButton` Back button.

- **New shared/reusable asset keys.** `game_asset_catalog.gd`/`asset_key_resolver.gd` gain `shared_background` (`ui_shared_background` → `res://assets/images/ui/shared/background.png`), `settings_window` (`ui_settings_window` → `res://assets/images/ui/settings/settings_window.png`), and `shared_back_button_default`/`shared_back_button_pressed` (`ui_shared_back_button_default`/`ui_shared_back_button_pressed` → `res://assets/images/ui/shared/buttons/back_default.png`/`back_pressed.png`). None of these image files ship yet — the empty `assets/images/ui/shared/`, `assets/images/ui/shared/buttons/`, and `assets/images/ui/settings/` folders exist with `.gitkeep` placeholders, and `GameAssetCatalog`/`ImageSlot`/`PressableTextureButton`'s existing missing-texture fallbacks apply (solid placeholder color, no crash) until real art is dropped in.
- **`SettingsScreen.tscn` rebuilt around a `SettingsWindow`.** The scene is now `SettingsScreen` (`Control`) → `Background` (`ImageSlot`, full-rect, bound to `shared_background`) and `SettingsWindow` (`ImageSlot`, bound to `settings_window`), sized 620x620 and centered on the 720x1280 portrait base resolution (`offset_left/top/right/bottom` = 50/330/670/950), freely resizable/movable from the Inspector. `SettingsWindow` holds `TitleLabel`, `SettingsContent` (`VBoxContainer` with `AnimationsRow`/`ReducedMotionRow`/`MusicRow`/`SoundEffectsRow`), and `BackButton`.
- **Visible settings unchanged in behavior.** Animations, Reduced Motion, Music, and Sound Effects stay `CheckButton` toggles wired to the same `SettingsManager` setters (`set_animations_enabled`, `set_reduced_motion_enabled`, `set_music_enabled`, `set_sound_effects_enabled`) and the same `toggle_on`/`toggle_off` asset binding as before.
- **Debug Labels hidden from the user-facing UI, not removed.** `DebugLabelsRow` stays in `SettingsContent` with `visible = false`; `DebugLabelsToggle` is still looked up (`get_node_or_null("%DebugLabelsToggle")`) and still refreshed/wired if present, but `settings_screen.gd` no longer assumes it exists — `SettingsManager`/`PlayerSettings.debug_labels_enabled` support is fully intact.
- **`BackButton` is now a `PressableTextureButton`.** It lives inside `SettingsWindow`, horizontally centered near the bottom (offset_left/top/right/bottom = 180/480/440/570 in window-local coordinates, ~260x90, ~50px above the window's lower edge). Its `normal_texture`/`pressed_texture` are bound at runtime from `shared_back_button_default`/`shared_back_button_pressed` only when the Inspector hasn't already assigned a texture (mirrors the `main_menu_screen.gd` fallback pattern), so it never overwrites hand-set art. `SettingsScreen` now connects to `back_button.delayed_pressed` (not `pressed`), so `back_pressed` fires only after the existing press-animation flow completes.
- **`App` return navigation unchanged.** `MainMenu → SettingsScreen → Back → MainMenu` and `LevelSelect → SettingsScreen → Back → LevelSelect` still work off the same `_settings_return_screen` field and `set_settings_manager`/`back_pressed` contract in `app.gd` — no routing code changed.
- **Unchanged by design.** `MainMenuScreen`, `LevelSelectScreen`, `ShopScreen`, `GameScreen`, star milestone rewards, global booster spending, wallet/booster inventory save data, deterministic layouts/boosts, fixed enemy HP/move curve, holes, ice, no-move shuffle, and the victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected.

## Stage 63.2.1: Editor-visible Settings Assets v0.1

Stage 63.2.1 makes `SettingsScreen` fully editor-visible, following the same pattern `MainMenuScreen` already used (Stage 63.1.2): backgrounds/window/button art are assigned directly in `SettingsScreen.tscn` via `Texture2D` `ext_resource`s instead of only appearing through runtime `ImageSlot`/`UiAssetBinding` calls, and runtime asset binding becomes fallback-only.

- **Direct `Texture2D` ext_resources added.** `SettingsScreen.tscn` now declares `ext_resource`s for `res://assets/images/ui/shared/background.png`, `res://assets/images/ui/settings/settings_window.png`, `res://assets/images/ui/shared/buttons/back_default.png`, and `res://assets/images/ui/shared/buttons/back_pressed.png`. Three of these files don't exist on disk yet; Godot tolerates a missing `ext_resource` target by loading the scene normally and leaving that one `texture`/`normal_texture`/`pressed_texture` property `null` (confirmed with a headless load test), so the scene stays safe today and needs no Inspector rework once each PNG is dropped at its declared path — Godot picks it up automatically on the next load. `background.png` already exists in the repo as of this stage and now renders directly in the editor.
- **`Background` is now a `TextureRect`.** Replaces the Stage 63.2 `ColorRect`/`ImageSlot`, with `texture = ExtResource(...)` assigned directly, full-rect anchors, `mouse_filter = 2` (ignore), `expand_mode = 1`, `stretch_mode = 5` — the same expand/stretch values `MainMenuScreen`'s `Background` uses for full-bleed portrait scaling.
- **`SettingsWindow` is now a `TextureRect`.** Same conversion, `texture = ExtResource(...)` assigned directly, keeping its ~620x620 size and centered position (`offset_left/top/right/bottom` = 50/330/670/950) and all of its existing children (`TitleLabel`, `SettingsContent`, `BackButton`) unchanged.
- **`BackButton` textures assigned directly in the scene.** `normal_texture`/`pressed_texture` are set via `ExtResource` on the `PressableTextureButton` node itself (mirroring `MainMenuScreen`'s buttons), so `ButtonTexture` previews `normal_texture` through `PressableTextureButton`'s existing `@tool` editor preview; child structure (`Placeholder`/`ButtonTexture`/`TextMargin`/`TextMargin/Label`) is unchanged.
- **Runtime binding is fallback-only.** `settings_screen.gd` drops its `ImageSlot`/`UiAssetBinding` calls for the background/window in favor of `background_rect: TextureRect`/`settings_window_rect: TextureRect` and two new methods, `_apply_background_texture()`/`_apply_settings_window_texture()`, each a direct port of `main_menu_screen.gd`'s `_apply_background_texture()` pattern: return immediately if the node's `texture` is already non-null (an Inspector-assigned or `ext_resource`-resolved texture), otherwise try `GameAssetCatalog.try_load_texture_cached()` and only assign if that lookup succeeds. `_bind_back_button_textures()` already followed this same only-if-null pattern from Stage 63.2 and needed no change. Toggle icon binding (`toggle_on`/`toggle_off` via `UiAssetBinding`) is untouched, since those aren't part of this stage's editor-visible-asset scope.
- **Settings behavior and App return navigation preserved.** `SettingsManager` injection, all four visible toggles (Animations/Reduced Motion/Music/Sound Effects), `AudioManager` application, the hidden/null-safe Debug Labels support, and `back_pressed` firing only after `BackButton.delayed_pressed` are all unchanged from Stage 63.2; `App`'s `_settings_return_screen` routing (`MainMenu ↔ SettingsScreen`, `LevelSelect ↔ SettingsScreen`) is untouched.
- **Unchanged by design.** `MainMenuScreen`, `LevelSelectScreen`, `ShopScreen`, `GameScreen`, economy, rewards, booster spending, deterministic layouts/boosts, holes, ice, no-move shuffle, and the victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected.

## Stage 63.3: Asset-Based Shop UI Shell v0.1

Stage 63.3 rebuilds `ShopScreen` from the technical `ColorRect`/`SafeMargin`/`VBoxContainer`/text-heavy-`ShopItemCard` layout into an asset-based shop UI shell that follows the same pattern `SettingsScreen` established in Stage 63.2/63.2.1: a shared background, a centered ~620x620 `ShopWindow`, a reused asset-based Back button, and visual product tiles instead of title/description/reward text blocks.

- **New reusable `FallbackImageSlot` component.** `scripts/ui/fallback_image_slot.gd` is a `@tool`-annotated `Control` (same child-ownership pattern as `PressableTextureButton`): it exposes a `texture` export and shows it via an internal `TextureDisplay` `TextureRect` when assigned, or a solid `placeholder_color` `Fallback` `ColorRect` when not — visible both in the editor and at runtime, and never overwritten by runtime code once a texture is present. `ShopScreen`'s `Background`/`ShopWindow` and every booster/product tile's icon slot use it, replacing one-off `ImageSlot`/`TextureRect` patterns with a single shared implementation.
- **New shop asset keys.** `game_asset_catalog.gd`/`asset_key_resolver.gd` gain `shop_window` (`ui_shop_window` → `res://assets/images/ui/shop/shop_window.png`), three booster icon keys (`res://assets/images/ui/icons/boosters/{hammer,freeze_time,rocket_barrage}.png`), four gem-product icon keys (`res://assets/images/ui/shop/gems/gems_{50,150,250,500}.png`), and four bundle icon keys (`res://assets/images/ui/shop/bundles/bundle_{small,medium,large,mega}.png`). None of these image files ship yet, so every one of these slots renders its colored fallback today; dropping a PNG at its declared path makes it appear automatically, in the editor and at runtime, with no scene/script changes needed.
- **`ShopScreen.tscn` rebuilt around a `ShopWindow`.** The scene is now `ShopScreen` (`Control`) → `Background` (`FallbackImageSlot`, full-rect, `shared_background`, texture assigned directly via `ext_resource` since `background.png` already exists) and `ShopWindow` (`FallbackImageSlot`, ~620x620, centered on the 720x1280 base resolution at `offset_left/top/right/bottom` = 50/330/670/950, no texture assigned since `shop_window.png` doesn't exist yet — its panel-color fallback shows instead). All shop content lives inside `ShopWindow`: `TabRow`, `WalletLabel`, `ContentRoot` (`BoostersContent`/`GemsContent`/`BundlesContent`/`OffersContent`), `FeedbackLabel`, and `BackButton`.
- **`BackButton` reuses the shared Back textures.** Same `PressableTextureButton` + `shared_back_button_default`/`shared_back_button_pressed` art as `SettingsScreen`, placed inside `ShopWindow` bottom-center (`offset_left/top/right/bottom` = 180/480/440/570, ~260x90, ~50px above the window's lower edge), connected to `delayed_pressed` so `back_pressed` fires only after the press animation completes.
- **Four top tabs, including a new Offers tab.** `TabRow` holds `BoostersTabButton`/`GemsTabButton`/`BundlesTabButton`/`OffersTabButton` (Бустеры/Гемы/Пакеты/Предложения) in one tight `HBoxContainer` (540px usable width, ~135px per tab, zero separation) near the top of `ShopWindow`; the active tab is full-brightness (`modulate = Color(1,1,1,1)`), inactive tabs are dimmed (`Color(0.55,0.55,0.55,1)`), matching the old tab-visual convention. `ShopItemCategory` gains `OFFERS = "offers"` in `ALL_IDS`.
- **Text-heavy `ShopItemCard` retired from `ShopScreen` in favor of visual tiles.** Two new components — `ShopBoosterTile` (`scenes/ui/shop/ShopBoosterTile.tscn` + `scripts/ui/shop/shop_booster_tile.gd`) and `ShopProductTile` (`scenes/ui/shop/ShopProductTile.tscn` + `scripts/ui/shop/shop_product_tile.gd`) — show only a square `FallbackImageSlot` icon plus buy controls, no title/description/reward text. `ShopItemCard.tscn`/`shop_item_card.gd` are left in place but are no longer referenced by `ShopScreen`.
- **Boosters tab: 2 rows × 3 tiles.** Top row (`TopRow`) is gold-priced (`booster_hammer_gold`/`booster_freeze_time_gold`/`booster_rocket_barrage_gold`), bottom row (`BottomRow`) is gem-priced (`booster_hammer_gems`/`booster_freeze_time_gems`/`booster_rocket_barrage_gems`). Each `ShopBoosterTile` has a `SpinBox` quantity control (default 1, min 1, max 99, integer steps) whose `value_changed` signal live-updates the buy button text to the total price (e.g. `"60 Gold"` for 3× a 20-gold booster).
- **`ShopPurchaseResolver.purchase()` gains a `quantity` parameter.** New signature: `purchase(item_id: String, progress_manager, shop_catalog, quantity: int = 1) -> Dictionary`. `quantity` is clamped to a minimum of 1 (`max(1, quantity)`); for `CURRENCY`-kind items, total price is `item.price_amount * quantity` and every reward amount is multiplied by `quantity` before being granted; insufficient currency still rejects with `not_enough_gold`/`not_enough_gems` against the *total* price. The result dictionary now also reports `"quantity"`. Existing single-item callers are unaffected since `quantity` defaults to `1`. `EXTERNAL_PAYMENT` items ignore quantity entirely, as before.
- **Gems tab: 2×2 grid, no quantity.** `gems_50`/`gems_150`/`gems_250`/`gems_500`, each a `ShopProductTile` with a square icon and a single buy button labeled with the item's display name (e.g. `"150 Gems"`). Pressing buy still calls `ShopPurchaseResolver.purchase()` and still returns `payment_not_connected` — no progress changes, matching Stage 62.4/62.5 behavior.
- **Bundles tab: 2×2 grid, no quantity.** `bundle_small`/`bundle_medium`/`bundle_large`/`bundle_mega`, same `ShopProductTile` treatment and same `payment_not_connected` result as Gems.
- **Offers tab: static placeholder, no catalog items.** `OffersContent` shows a centered two-line label ("Скоро" / "Предложения скоро появятся."); no purchases are wired for this stage.
- **Compact wallet display.** `WalletLabel` is a single label (`"Gold: %d   Gems: %d"`) placed inside `ShopWindow` below `TabRow` and above `ContentRoot`, refreshed after `set_progress_manager()` and after every purchase attempt (accepted or rejected), same as before.
- **`App` routing unchanged.** `ShopScreen` still only needs `set_progress_manager()`/`back_pressed`, exactly as `app.gd`'s `_show_shop_screen()` already expects — no `App`/routing changes were needed.
- **Verified with headless Godot.** A temporary scratch script instantiated `ShopScreen.tscn`, switched all four tabs, changed a booster tile's quantity to 3 and confirmed the buy button showed `"60 Gold"`, executed the purchase and confirmed gold dropped by exactly 60 with a `"Purchased!"` feedback message, and pressed a gem-product buy button and confirmed gems were unchanged with a `"Payments are not connected yet"` feedback message. The script was deleted after verification; it is not part of the repo.
- **Unchanged by design.** `MainMenuScreen`, `SettingsScreen`, `LevelSelectScreen`, `GameScreen`, star milestone rewards, global booster spending, wallet/booster inventory save data, deterministic layouts/boosts, fixed enemy HP/move curve, holes, ice, no-move shuffle, and the victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected.

## Stage 63.3.1: Shop Window Fallback Layering Hotfix v0.1

Stage 63.3.1 fixes a Stage 63.3 regression where `ShopScreen` rendered as an apparently empty window: `ShopWindow` itself had `FallbackImageSlot` attached directly, and `FallbackImageSlot`'s runtime-created `Fallback`/`TextureDisplay` children were `add_child()`-ed *after* `ShopWindow`'s tscn-declared `TabRow`/`WalletLabel`/`ContentRoot`/`FeedbackLabel`/`BackButton` — later siblings draw on top in Godot, so the fallback panel color (and any future `shop_window.png` texture) covered every tab, label, tile, and the Back button.

- **`ShopWindow` is now a plain layout parent.** `FallbackImageSlot`'s `script`/`texture`/`placeholder_color`/`expand_mode`/`stretch_mode` were removed from the `ShopWindow` node in `ShopScreen.tscn`; it's a bare `Control` again, keeping its ~620x620 rect (`offset_left/top/right/bottom` = 50/330/670/950) and every existing child.
- **New `WindowVisual` child carries the fallback/texture layer.** `WindowVisual` (`Control` + `FallbackImageSlot`) is now the *first* child of `ShopWindow` — full-rect anchors, `mouse_filter = 2` (ignore), same `placeholder_color = Color(0.14, 0.16, 0.22, 1)` and `shop_window` asset-key binding the old `ShopWindow` node used. Because it's declared before `TabRow` in the scene, its `Fallback`/`TextureDisplay` children are always the earliest-added (bottommost-drawn) nodes in `ShopWindow`, regardless of what order `FallbackImageSlot._ensure_children()` creates them in.
- **`shop_screen.gd` updated to match.** `shop_window_rect` was renamed `shop_window_visual` and now resolves `%WindowVisual` instead of `%ShopWindow`; `_bind_static_ui_assets()`'s `shop_window` texture binding is otherwise unchanged.
- **Resulting child order inside `ShopWindow`:** `WindowVisual`, `TabRow`, `WalletLabel`, `ContentRoot`, `FeedbackLabel`, `BackButton` — confirmed with a headless scratch script (deleted after use) that walked `ShopWindow.get_children()` and printed exactly this order.
- **`Background` is untouched.** It has no interactive/content children stacked on top of it, so keeping `FallbackImageSlot` directly on `Background` is still safe and its own missing-texture fallback still renders correctly.
- **`BackButton` unaffected.** Still uses `normal_texture`/`pressed_texture` assigned directly from the shared back-button art, still previews via `PressableTextureButton`'s `@tool` `ButtonTexture`, still fires `back_pressed` only after `delayed_pressed` — and now actually renders above `WindowVisual` again.
- **Shop logic fully preserved.** Re-verified with the same headless scratch-script approach as Stage 63.3: all four tabs switch content correctly, a booster tile's quantity-3 purchase shows `"60 Gold"` and deducts exactly 60 gold with `"Purchased!"` feedback, and the wallet/Offers placeholder/gem/bundle tiles all behave as before. The script was deleted after verification; it is not part of the repo.
- **Unchanged by design.** `MainMenuScreen`, `SettingsScreen`, `LevelSelectScreen`, `GameScreen`, economy, rewards, booster spending, deterministic layouts/boosts, holes, ice, no-move shuffle, and the victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected.

## Stage 63.3.2: Expanded Shop Window Layout v0.1

Stage 63.3.2 expands `ShopWindow` from 620x620 to 620x820 and repositions every child to use the added vertical space, so booster/gem/bundle tiles have more breathing room without crowding the tab row, wallet label, feedback, or Back button.

- **`ShopWindow` resized to 620x820.** `offset_left/top/right/bottom` = 50/330/670/1150 on the 720x1280 base resolution — width stays 620, the top edge stays fixed at y=330 (not vertically centered), and only the bottom edge moved down by 200px.
- **`WindowVisual` follows automatically.** It already used full-rect anchors (`anchor_right/bottom = 1.0`, `grow_horizontal/vertical = 2`), so no property changes were needed for it to fill the new 620x820 area — confirmed via a headless rect check (`WindowVisual rect = [P: (0,0), S: (620, 820)]`). It remains the first child of `ShopWindow`, still rendering behind every UI control, and still shows the `shop_window` texture if assigned or its colored fallback if not.
- **`TabRow` moved to `offset_top/bottom` = 28/86** (was 20/74), keeping `offset_left/right` = 40/580 (540px usable width) and all four ~135px tabs (Бустеры/Гемы/Пакеты/Предложения) tightly adjacent.
- **`WalletLabel` moved to `offset_top/bottom` = 92/120** (was 82/110), same compact `"Gold: %d   Gems: %d"` text and horizontal extent as `TabRow`.
- **`ContentRoot` expanded to `offset_left/top/right/bottom` = 20/130/600/630** (was 20/118/600/440) — 580x500, up from 580x322. `BoostersContent`/`GemsContent`/`BundlesContent`/`OffersContent` all use full-rect anchors inside it, so they inherited the larger area with no property changes; `ShopBoosterTile` (176x190 min) and `ShopProductTile` (260x200 min) both fit comfortably in the taller rows with no size changes needed.
- **`FeedbackLabel` moved to `offset_top/bottom` = 645/675** (was 448/474), sitting cleanly between `ContentRoot` and `BackButton` with no overlap.
- **`BackButton` moved to `offset_left/top/right/bottom` = 180/680/440/770** (was 180/480/440/570) — still 260x90, still horizontally centered, still ~50px above the window's new lower edge (820 - 50 - 90 = 680). `normal_texture`/`pressed_texture`/`delayed_pressed` and the `ButtonTexture`/`TextMargin`/`TextMargin/Label` child structure are all unchanged.
- **No vertical zone overlap.** TabRow (28-86), WalletLabel (92-120), ContentRoot (130-630), FeedbackLabel (645-675), BackButton (680-770) — each zone has breathing room before the next.
- **No script/logic changes needed.** All repositioned nodes keep their existing `unique_name_in_owner` names, so `shop_screen.gd`'s `%`-based node lookups, tab switching, Offers placeholder, booster quantity purchases, wallet refresh, purchase feedback, and `payment_not_connected` behavior for gems/bundles are all untouched.
- **Verified with headless Godot.** A temporary scratch script (deleted after use, not part of the repo) instantiated `ShopScreen.tscn` and printed `ShopWindow`'s rect (`(50,330)` size `(620,820)`) and every child's rect matching the values above, then re-ran the same tab-switching/quantity-purchase/feedback checks as Stage 63.3/63.3.1: qty-3 booster purchase showed `"60 Gold"`, gold dropped by exactly 60 with `"Purchased!"` feedback, and `BackButton`'s global rect confirmed it sits at the bottom of the expanded window.
- **Unchanged by design.** `MainMenuScreen`, `SettingsScreen`, `LevelSelectScreen`, `GameScreen`, economy, rewards, booster spending, deterministic layouts/boosts, holes, ice, no-move shuffle, and the victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected.

## Stage 63.3.3: Shop Tab Texture Buttons v0.1

Stage 63.3.3 replaces the plain `Button` shop tabs (selected/inactive state driven by `modulate`) with a new asset-based `ShopTabButton` component, so the active tab shows a persistent selected texture instead of a brightness tint, matching the asset-based visual language `PressableTextureButton` already established for Background/ShopWindow/BackButton.

- **New `ShopTabButton` component.** `scripts/ui/shop/shop_tab_button.gd` is a `@tool Button` sibling to `PressableTextureButton`: exported `default_texture`/`selected_texture`/`button_text`/`is_selected`/`fallback_default_color`/`fallback_selected_color`/`text_font_size`. Unlike `PressableTextureButton`'s temporary 0.2s press animation, `is_selected` is a persistent state — `set_selected(true)` keeps `selected_texture` showing until deselected, with no timer involved. The native `pressed` signal fires normally on click; no `delayed_pressed`-style wrapper was added since tab switching doesn't need a press animation.
- **Same explicit child structure as `PressableTextureButton`.** Each `ShopTabButton` owns `Fallback` (`ColorRect`, full-rect), `ButtonTexture` (`TextureRect`, full-rect), `TextMargin` (`MarginContainer`, full-rect) → `TextMargin/Label`, all `mouse_filter = 2` (ignore). Child lookup/creation happens once in `_ready()`, never in a setter, so tscn-declared children are never raced or auto-renamed. A `Label` that already exists in the scene keeps its Inspector-configured alignment/margins/font — `_update_label()` only ever touches `.text`, mirroring `PressableTextureButton`'s rule.
- **Selected vs. default visual resolution.** `_update_visual()` picks `selected_texture if is_selected else default_texture`, assigns it to `ButtonTexture.texture`, and shows `Fallback` (colored `fallback_selected_color` or `fallback_default_color` to match) whenever the resolved texture is `null` — so a missing texture always renders as a distinctly-colored tab instead of a blank one, and the fallback color itself signals selected vs. inactive state even with no art at all.
- **New shared tab texture asset keys.** `game_asset_catalog.gd`/`asset_key_resolver.gd` gain `shop_tab_default` (`ui_shop_tab_default` → `res://assets/images/ui/shop/tabs/tab_default.png`) and `shop_tab_pressed` (`ui_shop_tab_pressed` → `res://assets/images/ui/shop/tabs/tab_pressed.png`). All four tabs share these same two textures — there's no per-tab art. Neither file exists on disk yet, so every tab shows its colored fallback today (Godot tolerates the missing `ext_resource` targets exactly as documented in Stage 63.2.1/63.3.1 — a console error, no crash); dropping the two PNGs at their declared paths makes every tab pick up real art automatically.
- **`ShopScreen.tscn`'s four tab nodes converted to `ShopTabButton`.** `BoostersTabButton`/`GemsTabButton`/`BundlesTabButton`/`OffersTabButton` keep their existing manually-positioned rects inside `ShopWindow` (each ~135x54, tightly adjacent near the top) and existing `unique_name_in_owner` names; only their `script`/exported properties and child structure changed. `default_texture`/`selected_texture` are assigned directly via `ext_resource` (same "assign now, resolve later" pattern as `SettingsWindow`/`BackButton`), and `is_selected` is set directly in the scene: `true` for `BoostersTabButton`, `false` for the other three — so opening the scene in the editor immediately shows Boosters as the selected tab and the rest as inactive, with no script execution required to see the correct starting state.
- **`shop_screen.gd` updated to match.** `boosters_tab_button`/`gems_tab_button`/`bundles_tab_button`/`offers_tab_button` are now typed `ShopTabButton` instead of `Button`. `_update_tab_visuals()` no longer touches `modulate`/`TAB_MODULATE_SELECTED`/`TAB_MODULATE_UNSELECTED` (removed); it now calls `set_selected(_selected_category == <category>)` on each of the four tab buttons. A new `_bind_tab_button_textures()` (called from `_bind_static_ui_assets()`) mirrors `_bind_back_button_textures()`'s fallback-only pattern: for each tab button, it only assigns `default_texture`/`selected_texture` from `GameAssetCatalog` when that property is still `null`, so it never overwrites an Inspector-/`ext_resource`-assigned texture.
- **Manual positioning workflow preserved.** All four tabs remain independent sibling nodes directly under `ShopWindow` (not inside a container), so each can still be freely moved/resized/relabeled from the Inspector, and `default_texture`/`selected_texture`/`fallback_default_color`/`fallback_selected_color` can all be reassigned per-tab if a future stage wants per-tab art.
- **Verified with headless Godot.** A temporary scratch script (deleted after use, not part of the repo) confirmed `BoostersTabButton.is_selected == true` on load with the rest `false`, confirmed `Fallback.visible == true` and its color matched `fallback_selected_color`/`fallback_default_color` correctly for both states, confirmed pressing `GemsTabButton` flipped `is_selected` on both tabs and showed `GemsContent`, and re-ran the Stage 63.3 purchase flow (quantity-2 booster purchase, wallet refresh, `"Purchased!"` feedback) with no regressions.
- **Unchanged by design.** `MainMenuScreen`, `SettingsScreen`, `LevelSelectScreen`, `GameScreen`, economy, rewards, booster spending, deterministic layouts/boosts, holes, ice, no-move shuffle, and the victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected.

## Stage 63.4: Level Select Map Zones v0.1

Stage 63.4 rebuilds `LevelSelectScreen`'s foundation for a future custom level map: zones now hold 5 levels instead of 10, and the 5 level buttons per zone are fixed, manually-positionable editor nodes instead of a dynamically-built vertical list of plain `Button`s.

- **`LevelZoneHelper.ZONE_SIZE` is now 5** (`scripts/game/config/level_zone_helper.gd`), down from 10. `get_zone_count()`, `get_zone_index_for_level_number()`, `get_zone_index_for_level_id()`, `get_level_range_for_zone()`, `format_zone_label()`, and `get_zone_unlock_level_id()` all still work unchanged off the smaller `zone_size` default — only the constant changed. New ranges: Zone 1 = levels 1-5, Zone 2 = 6-10, Zone 3 = 11-15, Zone 4 = 16-20 (and so on across the 100-level campaign). Zone unlocks now trigger one zone earlier in level-count terms: Zone 2 unlocks after `level_5`, Zone 3 after `level_10`, Zone 4 after `level_15`.
- **New `LevelMapButton` component.** `scripts/ui/level_select/level_map_button.gd` is a `@tool Button`, structurally a sibling of `ShopTabButton`: exported `locked_texture`/`open_texture`/`completed_texture`/`pressed_texture`, `level_text`, `state` (`"locked"`/`"open"`/`"completed"` via `@export_enum`), `fallback_locked_color`/`fallback_open_color`/`fallback_completed_color`/`fallback_pressed_color`, and `text_font_size`. Same explicit child structure as `ShopTabButton`/`PressableTextureButton`: `Fallback` (`ColorRect`, full-rect), `ButtonTexture` (`TextureRect`, full-rect), `TextMargin` (`MarginContainer`, full-rect) → `TextMargin/Label`, all `mouse_filter = 2` (ignore), children looked up/created once from `_ready()` (never from a setter) so tscn-declared children are never raced/renamed, and an existing scene `Label`'s Inspector alignment/margins stay the source of truth. `pressed_texture` is exported and wired into `_get_active_texture()`'s state table but not yet swapped in on click — that delayed-press behavior is left for a later stage per this stage's scope.
- **State-driven visual resolution.** `_update_visual()` resolves `locked_texture`/`open_texture`/`completed_texture` based on `state`, assigns it to `ButtonTexture.texture`, and shows `Fallback` (colored to match the active state) whenever that texture is `null` — so every button always renders as a distinctly-colored slot even with zero art on disk, and updates live in the editor as soon as `state`/`level_text`/any texture export changes.
- **New level-select asset folders and keys.** `assets/images/ui/level_select/`, `assets/images/ui/level_select/zones/`, and `assets/images/ui/level_select/buttons/` were added (each with a `.gitkeep` since none of the folders hold real files yet). `asset_key_resolver.gd`/`game_asset_catalog.gd` gained/repointed keys: `level_select_background` → `res://assets/images/ui/level_select/background.png`, `level_select_zone_001_map` → `.../zones/zone_001_map.png`, `level_button_default` → `.../buttons/level_default.png`, `level_button_locked` → `.../buttons/level_locked.png`, `level_button_completed` → `.../buttons/level_completed.png`, and the new `level_button_pressed` → `.../buttons/level_pressed.png`. None of these files exist yet; `GameAssetCatalog.try_load_texture_cached()` returns `null` safely for all of them, same as every other reserved key in this project — no crash, no exception, just a colored fallback until real art is dropped at the declared paths.
- **`LevelSelectScreen.tscn` rebuilt around 5 fixed map slots.** The old `SafeMargin/Root/LevelScroll/LevelButtons` (`ScrollContainer`/`VBoxContainer`) that dynamically hosted N plain `Button`s is gone. The new structure is `Background` (`FallbackImageSlot`, full-rect) → `SafeMargin/Root` (top bar with `BackButton`/`TitleLabel`/`SettingsButton`, `PointsLabel`, `ZoneRow` with `ZoneLabel`/`ZoneSelector`) → `MapRoot` (`Control`, fills the remaining space below the top bar) containing `ZoneMapVisual` (`FallbackImageSlot`, full-rect) and five independent sibling nodes `LevelButton1`..`LevelButton5` (`LevelMapButton`), each individually positioned/sized (`offset_*`) so they can be freely dragged, resized, and relabeled per-button in the Inspector once real map art exists — no container manages their layout.
- **Dynamic level button creation replaced with fixed-slot refreshing.** `level_select_screen.gd`'s old `_build_level_buttons()` (which `queue_free()`d and recreated a `Button` per level every zone change) is replaced by `_refresh_level_button_slots()`, which updates the 5 existing `LevelMapButton` nodes in place: for each slot, `level_number = zone_range.x + slot_index`; if that level exists, the slot becomes `visible = true`, gets `level_text` set to the level number (or `"N (level_id)"` when Debug Labels is on), gets `state` set to `"locked"`/`"open"`/`"completed"` from existing `_is_level_unlocked()`/`_is_level_completed()`, gets `disabled` set to `not unlocked`, and star count from `_get_level_stars()` is stashed in `set_meta("level_stars", ...)`/`set_meta("star_asset_keys", ...)` for a future visual pass; if no level exists at that slot (short zone tail), the slot is simply `visible = false`. No node is ever created or freed during a zone switch.
- **Level selection signal preserved.** Each `LevelMapButton.pressed` is connected once in `_ready()` (bound to its slot index); `_on_level_button_pressed()` looks up that slot's current `level_id`, plays the level-select sound, and emits `level_selected(level_id)` only when the slot holds an unlocked level — a press on a locked or empty slot is a no-op, matching the previous `disabled`-gated behavior.
- **`ZoneSelector` stays for this stage.** Still the existing `OptionButton` in `ZoneRow`, still lists only unlocked zones and re-runs `_refresh_level_button_slots()` on selection — a custom prev/next map control is left for a later stage per this stage's explicit scope.
- **Editor-visible fallbacks everywhere.** `Background` and `ZoneMapVisual` are `FallbackImageSlot`s (`@tool`, same component `ShopScreen`/`SettingsScreen` already use): each shows its assigned `texture` when present and a colored `Fallback` rect otherwise, live in the editor. All 5 `LevelMapButton`s behave the same way per-state. Since none of Stage 63.4's declared texture paths have files on disk yet, every slot currently renders via its fallback color; dropping a PNG at the exact declared path (and reopening/reassigning in the Inspector, or via the runtime `GameAssetCatalog` binding) makes the fallback disappear and the texture appear with no code changes.
- **Progress/unlock/navigation behavior preserved.** `_is_level_unlocked()`, `_is_level_completed()`, `_get_level_stars()`, `_refresh_points()`, zone-unlock derivation (`_is_zone_unlocked()`), `BackButton` → `back_pressed`, `SettingsButton` → `settings_pressed`, and `App`'s existing LevelSelect routing are all unchanged.
- **Unchanged by design.** `MainMenuScreen`, `SettingsScreen`, `ShopScreen`, `GameScreen`, economy, rewards, booster spending, deterministic layouts/boosts, fixed enemy HP/move curve, holes, ice, no-move shuffle, and the victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected; `scripts/tests/level_select_zones_test.gd` and other LevelSelect-adjacent tests were intentionally left untouched per this stage's scope and may need a future update to match the new zone size and node structure.

## Stage 63.4.1: Unified Level Select Manual Layout v0.1

Stage 63.4.1 refactors `LevelSelectScreen` into a single manual layout canvas: the old `SafeMargin`/`Root`/`TopBar`/`ZoneRow` auto-layout containers (`MarginContainer`/`VBoxContainer`/`HBoxContainer`) are gone, replaced by one full-screen `LevelSelectCanvas` (`Control`) holding every visual element as an independently, manually positioned sibling node in the same 720x1280 coordinate space — so `Background`, `ZoneMapVisual`, and all 5 `LevelButton`s can be freely repositioned against each other and a future custom map texture without container reflow fighting the placement.

- **`SafeMargin`/`Root`/`TopBar`/`ZoneRow` removed.** These `MarginContainer`/`VBoxContainer`/`HBoxContainer` auto-layout nodes conflicted with manually positioning buttons/labels over a custom background texture — a container-managed top bar and a manually-positioned map don't share one coordinate space. `MapRoot`'s `offset_top = 230.0` wrapper is also gone; its function (starting the map area below the top UI) is now just `ZoneMapVisual`'s own `offset_top = 230.0`.
- **New `LevelSelectCanvas`.** `LevelSelectScreen.tscn` is now `LevelSelectScreen` (`Control`) → `LevelSelectCanvas` (`Control`, full-rect anchors, `mouse_filter = 2`) → every other node as a direct child: `Background`, `ZoneMapVisual`, `BackButton`, `SettingsButton`, `TitleLabel`, `PointsLabel`, `ZoneLabel`, `ZoneSelector`, `LevelButton1`..`LevelButton5`. All 13 children use explicit `offset_left/top/right/bottom` (`layout_mode = 1`) instead of container-managed sizing, so every element can be dragged/resized independently in the Inspector.
- **`Background` is the single base visual layer.** Unchanged behavior — full-rect `FallbackImageSlot`, first child of `LevelSelectCanvas` so every other node draws on top of it.
- **`ZoneMapVisual` and `LevelButton1`-`5` share the same coordinate space as `Background`.** All are now direct siblings under `LevelSelectCanvas` instead of being nested inside the removed `MapRoot`; each `LevelButton`'s offsets were shifted by the `+230` that `MapRoot`'s old `offset_top` used to contribute, so every button's on-screen position is visually unchanged from Stage 63.4 (`LevelButton1` 90,790→230,930; `LevelButton2` 300,690→440,830; `LevelButton3` 90,590→230,730; `LevelButton4` 300,490→440,630; `LevelButton5` 90,390→230,530). `ZoneMapVisual` keeps its own `offset_top = 230.0` with full-rect anchors otherwise, so it still fills the map area below the top UI without needing a parent offset.
- **Top UI elements moved to manual offsets.** `BackButton`, `SettingsButton`, `TitleLabel`, `PointsLabel`, `ZoneLabel`, and `ZoneSelector` are now direct `LevelSelectCanvas` children with their own `offset_*` rects (roughly preserving the old visual order: Back top-left, Settings top-right, Title centered between them, Points below, Zone label/selector below that) instead of living inside `TopBar`/`ZoneRow` `HBoxContainer`s — prioritizing easy manual repositioning over exact pixel-perfect placement, per this stage's scope.
- **`level_select_screen.gd` updated.** `root_panel` (`%Root`) and its `level_select_panel` asset binding were removed since the `Root` node no longer exists. Every other `@onready` lookup (`back_button`, `settings_button`, `points_label`, `zone_selector`, `background_slot`, `zone_map_visual`, `level_button_1`..`5`) needed **no changes** — they all resolve via scene-unique `%Name` lookups, which work regardless of which parent a node lives under, so reparenting under `LevelSelectCanvas` was transparent to the script.
- **Fixed 5-level zone slot logic fully preserved.** `LEVEL_SLOT_COUNT = 5`, `_level_buttons = [level_button_1..5]`, and `_refresh_level_button_slots()` are untouched — slot 0 still maps to the first level in the selected zone's range and slot 4 to the fifth, with locked/open/completed state, `disabled`, `level_text`, and star metadata assignment all unchanged from Stage 63.4.
- **Zone size 5 preserved.** `LevelZoneHelper.ZONE_SIZE` stays 5; Zone 1 = levels 1-5, Zone 2 = 6-10, Zone 3 = 11-15, Zone 2 unlocks after `level_5`, Zone 3 after `level_10` — all unchanged from Stage 63.4.
- **Editor-visible fallback behavior preserved.** `Background`, `ZoneMapVisual`, and all 5 `LevelButton`s are still `@tool` fallback components (`FallbackImageSlot`/`LevelMapButton`); none of their texture/fallback logic changed, only their parent node.
- **Progression/selection/navigation signals preserved.** `level_selected(level_id)`, `settings_pressed`, `back_pressed`, and the `ZoneSelector`-driven zone refresh are all unchanged.
- **Unchanged by design.** `MainMenuScreen`, `SettingsScreen`, `ShopScreen`, `GameScreen`, economy, rewards, booster spending, deterministic layouts/boosts, holes, ice, no-move shuffle, and the victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected; `ui_asset_key_binding_test.gd`'s `%Root`/`level_select_panel` assertion was intentionally left untouched per this stage's test-freeze scope and will need a future update since the `Root` node no longer exists.
- **Hotfix: `ZoneMapVisual` removed as a duplicate background layer.** After manual testing, having `ZoneMapVisual` as a second full-rect `FallbackImageSlot` stacked on `Background` (starting at `offset_top = 230.0`) produced a visible "nested image" artifact whenever a real background texture was assigned — the same texture rendered twice in two differently-sized rects, each with independent `KEEP_ASPECT_CENTERED` scaling, so the lower `ZoneMapVisual` copy appeared as a cropped/zoomed box over the full-screen `Background`. Since every zone shares one background image (there is no per-zone map art), `ZoneMapVisual` was removed entirely from `LevelSelectScreen.tscn`, along with `level_select_screen.gd`'s `zone_map_visual` `@onready` var and its texture binding, and the now-unused `level_select_zone_001_map`/`ui_level_select_zone_001_map` asset key was removed from `asset_key_resolver.gd`/`game_asset_catalog.gd`/`docs/ASSET_MAP.md`. `Background` is now the single, unambiguous background layer for the whole screen; `LevelButton1`-`5` sit directly on top of it with no intermediate map layer.

## Stage 63.4.2: Level Button Overlays and Level Info Popup v0.1

Stage 63.4.2 reworks `LevelMapButton`'s visual states from three separate full textures into one shared base texture with layered locked/completed overlays and color filters, and replaces "press a level to start it immediately" with a centered `LevelInfoPopup` (level number, earned stars, Start/Back) that gates the actual `level_selected` emission behind a Start press.

- **`LevelMapButton` now renders every state on top of one `default_texture`.** `scripts/ui/level_select/level_map_button.gd`'s exported `locked_texture`/`open_texture`/`completed_texture` are replaced with `default_texture` (always shown) plus `locked_overlay_texture`/`completed_overlay_texture` (optional, layered on top only for their state) and the existing `pressed_texture` (still reserved, unused for click behavior). `open` shows only `default_texture` with no filter/overlay; `locked` and `completed` add a translucent color filter (new `locked_filter_color` default `Color(0, 0, 0, 0.45)`, `completed_filter_color` default `Color(0.1, 0.8, 0.25, 0.30)`) plus their overlay texture if one is assigned — the filter alone is always enough to distinguish locked/completed even with zero overlay art, so a missing overlay texture never hides the state.
- **New child layer order.** Each `LevelMapButton` now owns `Fallback` → `ButtonTexture` → `StateFilter` (new `ColorRect`) → `StateOverlay` (new `TextureRect`) → `TextMargin` → `TextMargin/Label`, all `mouse_filter = 2` (ignore), following the same once-only `_ready()` child lookup/creation rule as `ShopTabButton`/`PressableTextureButton` — `StateFilter`/`StateOverlay` are auto-created the first time each existing `LevelButton1`-`5` node (already in the user's manually-tuned scene) opens in the editor, with no manual scene surgery needed. `Fallback`'s single `fallback_color` (replacing the old per-state `fallback_locked_color`/`fallback_open_color`/`fallback_completed_color`) is now keyed only to whether `default_texture` is missing, matching `FallbackImageSlot`'s "one placeholder for one texture slot" convention.
- **Asset paths updated.** New keys `level_button_locked_overlay`/`level_button_completed_overlay` (`ui_level_button_locked_overlay`/`ui_level_button_completed_overlay` → `res://assets/images/ui/level_select/buttons/level_locked_overlay.png`/`level_completed_overlay.png`) were added to `asset_key_resolver.gd`/`game_asset_catalog.gd`/`docs/ASSET_MAP.md`. The old `level_button_locked`/`level_button_completed` keys (full-texture paths `level_locked.png`/`level_completed.png`) are kept in the catalog for compatibility but are no longer read by `level_select_screen.gd` — documented in `ASSET_MAP.md` as legacy/unused since this stage.
- **`level_select_screen.gd`'s texture binding updated.** `_bind_static_ui_assets()` now loads `level_button_default`/`level_button_locked_overlay`/`level_button_completed_overlay`/`level_button_pressed` and assigns each to every `LevelButton1`-`5` only when that field is still `null`, so an Inspector-assigned texture on any individual button is never overwritten by the shared runtime binding.
- **New `LevelInfoPopup`.** Added as the last child of the existing `LevelSelectCanvas` (so it draws above the map/buttons) without touching any other existing manually-placed node: `LevelInfoPopup` (`Control`, full-rect, starts `visible = false`) → `Scrim` (`ColorRect`, semi-transparent, blocks clicks to the map while the popup is open) → `PopupWindow` (`FallbackImageSlot`, ~480x360 centered box, new `level_info_window` asset key → `res://assets/images/ui/level_select/level_info_window.png`) → `LevelTitleLabel`/`StarsLabel`/`StartButton`/`PopupBackButton`, all manually positioned siblings so the user can freely move/resize any of them, matching the rest of the scene's manual-layout convention. `PopupWindow` shows its texture when assigned and a colored fallback otherwise, exactly like `Background`.
- **Level press now opens the popup instead of starting the level.** `_on_level_button_pressed()` still early-returns for an empty slot or a locked level (so locked buttons never open the popup — `_is_level_unlocked()` remains the single source of truth), but on success now calls `_show_level_info_popup(level_id)` instead of emitting `level_selected` directly. The popup sets `_pending_level_id`, `LevelTitleLabel.text` to `"Level %d"` (via `LevelLabelFormatter`), and `StarsLabel.text` to a `"Stars: ★★☆"`-style string built by a small new `_format_stars_text()` helper from the existing `_get_level_stars()` — no new star-tracking state was introduced, it reuses the same progress data the button slots already display.
- **`StartButton` is the only thing that starts a level.** `_on_popup_start_button_pressed()` no-ops if `_pending_level_id` is empty (defensive only — the popup can't open without a valid pending id), otherwise hides the popup, plays the level-select sound, and emits `level_selected(level_id)` — identical side effects to the old direct-press behavior, just deferred behind the popup.
- **`PopupBackButton` only closes the popup.** `_on_popup_back_button_pressed()` plays the button-click sound and calls `_hide_level_info_popup()` (clears `_pending_level_id`, hides the popup) — it does not touch or emit the screen-level `back_pressed` signal, so it can never be confused with the top-bar Back button that returns to MainMenu.
- **Fixed 5-level zone slot logic untouched.** `LEVEL_SLOT_COUNT = 5`, `_level_buttons`, `_refresh_level_button_slots()`, locked/open/completed `state` assignment, `disabled`, `level_text`, and star metadata are all unchanged from Stage 63.4/63.4.1.
- **Current manually-tuned scene structure fully preserved.** No existing node was moved, renamed, or restructured — `LevelSelectCanvas`, `Background`, `BackButton`/`SettingsButton`/`TitleLabel`/`PointsLabel`/`ZoneLabel`/`ZoneSelector`, and each `LevelButton1`-`5`'s individually-tuned `offset_*` rect are all exactly as the user last positioned them; only `LevelInfoPopup` and its children were appended.
- **Unchanged by design.** `MainMenuScreen`, `SettingsScreen`, `ShopScreen`, `GameScreen`, economy, rewards, booster spending, deterministic layouts/boosts, holes, ice, no-move shuffle, and the victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected.
- **Hotfix: `StateFilter` now mirrors `default_texture`'s own alpha shape instead of being a plain rectangle.** `StateFilter` was originally a full-rect `ColorRect`, which visibly bled outside a round `default_texture`'s edges (a round button with transparent corners still got a square dark/green tint). `StateFilter` is now a `TextureRect` that renders `default_texture` again and is tinted via `modulate` (which multiplies the texture's own per-pixel alpha), so the locked/completed filter color only ever appears where the base texture itself is opaque — a round button stays round under the filter, with no square bleed.

## Stage 63.4.3: Level Select UI Cleanup and Shared Back Button v0.1

Stage 63.4.3 removes unused text/debug UI nodes from `LevelSelectScreen` and converts its `BackButton` to the same shared asset-based `PressableTextureButton` used by `SettingsScreen`/`ShopScreen`, while keeping the user's current manual scene structure otherwise untouched.

- **Removed nodes.** `TitleLabel`, `PointsLabel`, `ZoneLabel`, and `SettingsButton` are deleted from `LevelSelectScreen.tscn`. `LevelSelectCanvas`, `Background`, `BackButton`, `ZoneSelector`, `LevelButton1`-`5`, and `LevelInfoPopup`/its children are all preserved exactly as previously positioned.
- **Removed script references.** `level_select_screen.gd` drops the `settings_button`/`points_label` `@onready` refs, the `settings_button.pressed` connection, `_on_settings_button_pressed()`, `_refresh_points()`, and its call from `_refresh()`. The `settings_pressed` signal is still declared (so `App`'s existing `screen.settings_pressed.connect(...)` wiring keeps compiling) but is never emitted from `LevelSelectScreen` anymore since its trigger button is gone.
- **`BackButton` is now a `PressableTextureButton`.** It uses `scripts/ui/pressable_texture_button.gd` with the same `Placeholder`/`ButtonTexture`/`TextMargin`/`TextMargin/Label` child pattern as `SettingsScreen`'s/`ShopScreen`'s `BackButton`, text `"Назад"`, and is bound at runtime via a new `_bind_back_button_textures()` (mirroring `settings_screen.gd`'s helper) to the existing shared `shared_back_button_default`/`shared_back_button_pressed` asset keys (`res://assets/images/ui/shared/buttons/back_default.png`/`back_pressed.png`), only filling in a texture field when the Inspector hasn't already assigned one. If the shared textures are missing on disk, `PressableTextureButton`'s existing placeholder `ColorRect` keeps the button visible/editable.
- **Matched aspect ratio.** `BackButton`'s rect is resized to 260x90 (from the old 110x54 plain-`Button` size), matching the practical size family used by the Settings/Shop `BackButton`. It stays a manually positioned sibling inside `LevelSelectCanvas` so the user can still freely reposition it.
- **Delayed back navigation.** `level_select_screen.gd`'s `back_button` is now typed `PressableTextureButton` and connects `back_button.delayed_pressed` (not raw `pressed`) to `_on_back_button_pressed()`, which still plays the button-click sound and emits `back_pressed` — identical semantics to `SettingsScreen`'s Back button, so the texture swap/press animation plays before navigation happens.
- **`ZoneSelector` unchanged.** It remains a temporary `OptionButton` for zone switching; removing `ZoneLabel` does not affect its `item_selected` wiring or zone-building logic.
- **`LevelInfoPopup` and fixed 5-level zone logic untouched.** Popup open/close on open-or-completed vs. locked levels, `StartButton`/`PopupBackButton` behavior, `LEVEL_SLOT_COUNT = 5`, `LevelButton1`-`5` slot refresh, and locked/open/completed overlay/filter presentation are all unchanged from Stage 63.4.2.
- **Unchanged by design.** `MainMenuScreen`, `SettingsScreen`, `ShopScreen`, `GameScreen`, economy, rewards, booster spending, deterministic layouts/boosts, holes, ice, no-move shuffle, and the victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected.

## Stage 63.4.4: Level Info Popup Star-State Window and Shared Buttons v0.1

Stage 63.4.4 removes the text-based `StarsLabel` from `LevelInfoPopup` in favor of a `PopupWindow` background that itself changes by earned stars, and converts `StartButton`/`PopupBackButton` to the same shared asset-based `PressableTextureButton` used by the screen's Back button.

- **`StarsLabel` removed.** Deleted from `LevelInfoPopup` in `LevelSelectScreen.tscn`. `level_select_screen.gd` drops the `popup_stars_label` `@onready` ref, the line that wrote its text, and the now-unused `_format_stars_text()` helper. Stars are no longer shown as separate text at all — they're communicated purely by which `PopupWindow` texture is showing.
- **Four new star-state popup window asset keys.** `level_info_window_0_stars`/`level_info_window_1_star`/`level_info_window_2_stars`/`level_info_window_3_stars` (→ `ui_level_info_window_0_stars`/`..._1_star`/`..._2_stars`/`..._3_stars`) were added to `asset_key_resolver.gd`/`game_asset_catalog.gd`, resolving to `res://assets/images/ui/level_select/popup/level_info_{0_stars,1_star,2_stars,3_stars}.png`. The new `assets/images/ui/level_select/popup/` folder was created with a `.gitkeep` placeholder since none of the four PNGs exist yet. The old single `level_info_window` key stays in the catalog for compatibility but is unused since this stage (documented as legacy in `docs/ASSET_MAP.md`).
- **`PopupWindow` texture now depends on earned stars.** `_bind_static_ui_assets()` no longer statically binds `PopupWindow` to the old single `level_info_window` key; instead, a new `_apply_popup_window_texture(stars)` (called from `_show_level_info_popup()` with `_get_level_stars(level_id)`, via a `_get_popup_window_asset_id()` clamp-to-0..3 lookup) assigns the matching star-state texture to `popup_window_slot.texture` every time the popup opens for a level. When the matching texture is missing on disk, `try_load_texture_cached()` returns `null` and `FallbackImageSlot` shows its existing colored `Fallback` rect — same fallback-safe behavior as before, just re-evaluated per popup-open instead of once at `_ready()`.
- **`StartButton`/`PopupBackButton` are now `PressableTextureButton`s.** Both use `scripts/ui/pressable_texture_button.gd` with the same `Placeholder`/`ButtonTexture`/`TextMargin`/`TextMargin/Label` child pattern as the screen's shared `BackButton`, texts `"Начать"`/`"Назад"` respectively, and reuse the same `shared_back_button_default`/`shared_back_button_pressed` textures (`res://assets/images/ui/shared/buttons/back_default.png`/`back_pressed.png`) — a new shared `_bind_pressable_texture_button(button, normal_key, pressed_key)` helper replaces the old single-button `_bind_back_button_textures()` body and is now called for `back_button`, `popup_start_button`, and `popup_back_button` alike. Both buttons keep their existing manually-positioned rects inside `LevelInfoPopup` (only their node type/script/children changed), still freely movable/resizable in the editor.
- **Popup button presses gated behind `delayed_pressed`.** `popup_start_button`/`popup_back_button` are now typed `PressableTextureButton` and their `_ready()` connections switched from raw `.pressed` to `.delayed_pressed`, so `_on_popup_start_button_pressed()`/`_on_popup_back_button_pressed()` only fire after the press-texture swap animation finishes — matching the screen `BackButton`'s existing behavior.
- **Popup-first level launch flow preserved.** Locked levels still never open `LevelInfoPopup` (`_is_level_unlocked()` gate unchanged); `StartButton` still only starts `_pending_level_id` and is the sole emitter of `level_selected`; `PopupBackButton` still only hides the popup and clears `_pending_level_id`, never touching the screen-level `back_pressed` signal.
- **Unchanged by design.** Zones of 5 levels, `LevelButton1`-`5` fixed-slot logic, `ZoneSelector` (still temporary), the screen `BackButton`'s own shared-texture/delayed-press behavior, `MainMenuScreen`, `SettingsScreen`, `ShopScreen`, `GameScreen`, economy, rewards, booster spending, deterministic layouts/boosts, holes, ice, no-move shuffle, and the victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** Manual verification in the Godot editor is expected.
...
- **Dropdown popup styled via theme overrides.** New `_style_zone_selector_popup()` fetches `zone_selector.get_popup()` (a `PopupMenu`, safely no-op'ing if `null`) and applies `add_theme_font_size_override("font_size", 26)` plus `add_theme_constant_override(...)` for `v_separation` (18), `item_start_padding` (16), and `item_end_padding` (16) — sized against the current `ZoneSelector` node's own ~70px height (`offset_top = 40` to `offset_bottom = 110`) without hardcoding the node's own rect, which the user still adjusts manually in the editor. Called once from `_ready()` and again at the end of every `_build_zone_selector()` rebuild.
- **New zone star rating helper.** `_get_zone_rating_stars(zone_index: int) -> int` sums `_get_level_stars()` across the zone's level range (from `LevelZoneHelper.get_level_range_for_zone()`) for earned stars, and `existing_level_count * 3` for max stars; returns `3` only at exactly 100% (`earned == max`), `2` at 50-99%, and `1` below 50% — with `1` returned safely whenever the zone has no levels (`level_range == Vector2i.ZERO`) or `max_stars <= 0`, per explicit request that under-50% still shows a star rather than zero.
- **New star text formatter.** `_format_zone_stars(stars: int) -> String` clamps to `1..3` and renders `"★".repeat(n) + "☆".repeat(3 - n)` (e.g. `2` → `★★☆`).
- **Zone labels now show stars.** `_build_zone_selector()` appends `" " + _format_zone_stars(_get_zone_rating_stars(zone_index))` to the existing `LevelZoneHelper.format_zone_label()` text for every unlocked zone item — e.g. `"Zone 1: Levels 1-5 ★☆☆"` — with the label's existing wording/format otherwise untouched.
- **Zone stars stay current.** Since `refresh_progress_state()`/`_refresh()` already call `_build_zone_selector()` on every progress update, returning to LevelSelect after completing levels rebuilds the zone items (and their star text) from the latest `_progress_manager` data with no separate refresh path needed.
- **`ZoneSelector` selection/id behavior preserved.** Item ids are still `zone_index` (`zone_selector.add_item(label, zone_index)` unchanged); unlocked-zone list building, "stay on the current selection while it's still valid, otherwise default to the highest unlocked zone" logic, and `_on_zone_selected()` updating `_selected_zone_index` plus refreshing `LevelButton1`-`5` are all unchanged.
...
## Stage 64.1: Top Battle UI Rebuild v0.1

Stage 64.1 rebuilds the top part of `GameScreen` around the battle board: the top-to-bottom order becomes Level/Moves/Levels (`HudRow`), then `EnemyPanel`, then `RoundModifierPanel`, then the board. `EnemyPanel` is rebuilt into an asset-based visual panel with a background, a centered enemy sprite, a red HP bar, and an HP value label; `RoundModifierPanel` shows only the boost description. `BoardView` and the lower booster panel are unchanged.

- **`GameScreen.tscn` `BattleRoot` reordered.** `EnemyPanel` moved from the first child of `BattleRoot` to directly after `HudRow`, so the top order is now `HudRow` → `EnemyPanel` → `RoundModifierPanel` → `BoardArea` → `StatusLabel` → `HeroPartyPanel` → `BoosterPanel`. `HudRow`'s `BattleHud`/`MenuButton` and their behavior (level/moves display, `back_pressed` navigation) are unchanged.
- **`EnemyPanel.tscn` rebuilt.** The old `Margin` → `Row` (`EnemyImageSlot`, `Text` → `EnemyNameLabel`/`EnemyHpLabel`/`EnemyHpBar`/`EnemyIntentLabel`/`EnemyTargetLabel`) structure is replaced by `Layers` (a plain `Control`) holding: `BackgroundVisual` (`FallbackImageSlot`, full-rect, `STRETCH_KEEP_ASPECT_COVERED`), `EnemySprite` (`FallbackImageSlot`, full-rect, default `STRETCH_KEEP_ASPECT_CENTERED` so it stays centered and preserves aspect within the panel), `HpBarRoot` (a bottom-anchored `Control` holding `HpBarBackground`/`HpBarFill` `ColorRect`s and a `HpValueLabel`), and `HitEffectLayer` (unchanged role, now nested under `Layers`). All new visual nodes are `FallbackImageSlot`s or plain `ColorRect`s, so textures show in the editor when assigned and fall back to a colored rect otherwise.
- **Old enemy text removed from the visible UI.** `EnemyNameLabel`, the `"HP:"`-prefixed `EnemyHpLabel`, `EnemyIntentLabel`, and `EnemyTargetLabel` are gone. Visible enemy UI is now just the enemy visual, the red HP bar, and an HP value label centered inside the bar.
- **HP bar is red and label-driven.** `enemy_panel.gd`'s `set_hp_values(current_hp, max_hp)` clamps both values to non-negative, computes `ratio = current / max` (safely `0.0` when `max <= 0`), animates `HpBarFill.anchor_right` to the clamped ratio (respecting the existing animation/reduced-motion settings), and always sets `HpValueLabel.text` to `"current / max"` — e.g. `"130 / 130"`, `"100 / 130"`, `"0 / 130"` — with no `"HP:"` prefix. `animate_hp_change()` is kept as a thin wrapper over `set_hp_values()` for compatibility.
- **`set_enemy_state(enemy, intent)` preserved but simplified.** Still called by `game_screen.gd` exactly as before (`enemy_panel.set_enemy_state(state.enemy, state.enemy_intent)`); it now updates the HP bar/label and resolves the enemy sprite's texture, but the `intent`/target parameter is accepted only for signature compatibility and otherwise ignored, since no intent/target text is shown anymore.
- **Normal/damaged enemy sprite states.** New `set_enemy_textures(normal_texture, damaged_texture)` and `play_damage_feedback()` on `enemy_panel.gd`: the normal texture displays by default; `play_damage_feedback()` swaps in the damaged texture briefly (skipped if `EnemySprite`'s texture was manually assigned in the Inspector, tracked via a manual-override flag captured once in `_ready()`) then restores the normal texture, and falls back to the existing flash/shake `_play_flash_and_shake()` feedback when no damaged texture is available. `game_screen.gd`'s existing `_play_enemy_damage()` (already called after valid enemy damage in both the swap-turn and booster-turn flows) now also calls `enemy_panel.play_damage_feedback()` when the method exists, without touching the existing audio/particle/`play_hit_feedback()` flow or blocking board animation.
- **New enemy sprite/state asset keys.** `AssetKeyResolver.get_enemy_normal_asset_key(enemy_id)`/`get_enemy_damaged_asset_key(enemy_id)` currently resolve only `gatekeeper` → `gatekeeper_normal`/`gatekeeper_damaged`, added to `GameAssetCatalog` as `res://assets/images/ui/game/enemies/gatekeeper_normal.png`/`gatekeeper_damaged.png`. The new `assets/images/ui/game/enemies/` folder was created with a `.gitkeep` placeholder since neither file exists yet.
- **New enemy panel background asset keys.** `AssetKeyResolver.get_enemy_panel_background_asset_keys()` returns five keys `enemy_panel_bg_01`..`05`, added to `GameAssetCatalog` as `res://assets/images/ui/game/enemy_panel/backgrounds/enemy_panel_bg_0{1-5}.png`. `enemy_panel.gd`'s `_ready()` picks one at random via `try_load_texture_cached()` and assigns it to `BackgroundVisual` only if the Inspector hasn't already assigned a texture (tracked the same way as `EnemySprite`'s manual-override flag) — full battle-background-style randomization/reuse is left for later polish. The new `assets/images/ui/game/enemy_panel/backgrounds/` folder was created with a `.gitkeep` placeholder since none of the five PNGs exist yet.
- **`RoundModifierPanel` simplified.** `ModifierNameLabel` was removed from `GameScreen.tscn` and from `game_screen.gd`; `_on_level_boost_changed(boost)` now sets only `modifier_description_label.text = boost.description`. The null/`is_none()` boost still hides `RoundModifierPanel` exactly as before.
- **Layout code unchanged in practice.** `game_screen.gd`'s `_apply_portrait_layout()`/`_apply_landscape_layout()` still size `enemy_panel.custom_minimum_size` the same as before (`(0, 132)` portrait / `(0, 92)` landscape) — the internal rebuild doesn't require different outer sizing for this stage.
- **Unchanged by design.** `BoardView`, `StatusLabel`, `HeroPartyPanel`, `BoosterPanel`, booster buttons/inventory, match-3 board/input/swaps/cascades, damage calculation, HP changes, victory/defeat, no-move shuffle, ice, holes, deterministic levels, boosters, economy, rewards, `LevelSelectScreen`, `MainMenuScreen`, `SettingsScreen`, `ShopScreen` are all untouched.
- **No automated tests were added, updated, touched, or run.** `enemy_panel_hit_feedback_test.gd`, `enemy_panel_image_slot_test.gd`, and `game_screen_layout_test.gd` reference the old node/field names and will need a future update to match this stage's `EnemyPanel`/`RoundModifierPanel` structure; manual verification in the Godot editor is expected for this stage.

## Stage 64.2: Bottom Booster UI Rebuild v0.1

Stage 64.2 rebuilds the bottom part of `GameScreen` around the booster panel: the visible `StatusLabel` and `FreezeLabel` are removed from the battle layout, `BoosterPanel` is expanded, and booster controls become square texture-based `BoosterTextureButton`s with a bottom-center count overlay.

- **`StatusLabel` hidden from the battle layout.** `GameScreen.tscn`'s `StatusLabel` now has `visible = false`; as a hidden child of the `BattleRoot` `VBoxContainer` it no longer reserves vertical space between `BoardArea` and `BoosterPanel`, so `"Select a tile"` and other status text are no longer shown during normal gameplay. `game_screen.gd`'s `_set_status()` is unchanged and remains safe to call for internal/debug feedback, invalid input, and victory/defeat text — it just writes to a hidden `Label`.
- **`FreezeLabel` removed from `BoosterPanel`.** `BoosterPanel.tscn`'s old `Content` (`VBoxContainer`) → `ButtonRow`/`FreezeLabel` structure is replaced by `BoosterPanel` (`PanelContainer`) directly holding `PanelBackground` (`FallbackImageSlot`, full-rect, editor-visible/fallback-safe) and `ButtonRow` (`HBoxContainer`, `alignment = 1` for horizontal centering) as overlapping full-rect children — the same "multiple direct `PanelContainer` children stack" pattern `EnemyPanel` already uses. `booster_panel.gd` no longer reads or displays `freeze_turns_left`; the underlying `BoosterState.freeze_turns_left` data is untouched, it's just not shown as separate text anymore.
- **`BoosterPanel` expanded.** `game_screen.gd`'s `_apply_portrait_layout()`/`_apply_landscape_layout()` now size `booster_panel.custom_minimum_size` to `(width, 160)` portrait / `(width, 112)` landscape (previously `132`/`96`), using the space freed by the removed labels. `BoardView`'s own size (`PORTRAIT_BOARD_SIZE`/`LANDSCAPE_BOARD_SIZE`) is untouched.
- **New `BoosterTextureButton` component.** Added as `scripts/ui/booster_texture_button.gd` (`@tool`, `extends Button`, `class_name BoosterTextureButton`) plus a hand-editable template scene `scenes/ui/BoosterTextureButton.tscn`, following the same once-only `_ready()` child lookup/creation pattern as `FallbackImageSlot`/`LevelMapButton`. Children, in draw order: `Fallback` (`ColorRect`), `ButtonTexture` (`TextureRect`, `STRETCH_KEEP_ASPECT_CENTERED`), `StateFilter` (`ColorRect`, tint overlay), `CountLabel` (`Label`, bottom-center overlay, editable font/position in the template scene).
- **Exported properties drive all visual state.** `booster_id`, `default_texture`/`disabled_texture`/`selected_texture`, `count_text`, `is_selected`/`is_disabled_state`, `fallback_ready_color`/`fallback_disabled_color`/`fallback_selected_color`, and `disabled_filter_color`/`selected_filter_color` are all `@export`ed with setters that call `_update_visual()` (a no-op until `_ready()` has run, matching `FallbackImageSlot`'s safe-deserialization pattern). Active texture resolves as `selected_texture` (if `is_selected` and assigned) → `disabled_texture` (if `is_disabled_state` and assigned) → `default_texture`; a missing active texture shows `Fallback` in the matching state color (ready/disabled/selected); `StateFilter` additionally tints the button for disabled (priority) or selected state even when a texture is present, so state remains visible with zero icon art. `set_count(value)` formats `CountLabel.text` as `"x%d"` (e.g. `"x0"`, `"x1"`, `"x3"`, `"x10"`).
- **Square sizing without a hardcoded size.** `BoosterTextureButton.tscn`'s own `custom_minimum_size` (96x96) is just an editor-time default; `booster_panel.gd`'s new `_apply_button_square_size()` (called once via `call_deferred()` after rebuilding buttons, and again on every `resized` signal from the panel itself) sets every button's `custom_minimum_size` to a square sized from `button_row.size.y` (minus a small margin, floored at a minimum), so buttons automatically grow/shrink with the portrait/landscape `BoosterPanel` height. Each button's `size_flags_horizontal`/`size_flags_vertical` are `SIZE_SHRINK_CENTER` so `ButtonRow`'s `HBoxContainer` centers them instead of stretching them to fill the row.
- **Booster icon assets reused, not duplicated.** `BoosterTextureButton.default_texture` is populated from the existing `AssetKeyResolver.get_shop_booster_icon_asset_key(booster_id)` → `GameAssetCatalog` keys (`shop_icon_booster_hammer`/`_freeze_time`/`_rocket_barrage` → `res://assets/images/ui/icons/boosters/hammer.png`/`freeze_time.png`/`rocket_barrage.png`), which already existed for the Shop screen and already point at the exact paths this stage wanted — no new asset keys or duplicate art paths were added.
- **`booster_panel.gd` behavior contract preserved.** `setup_boosters(catalog)`, `set_booster_state(booster_state)`, `set_selected_booster(booster_id)`, `set_booster_counts(counts)`, `play_booster_feedback(booster_id, ...)`, `get_button_count()`, and the `booster_pressed(booster_id)` signal are all unchanged in signature and behavior. `refresh()` now calls `button.set_count(inventory_count)` and sets `button.is_selected`/`button.is_disabled_state` directly (equivalent to the old `BoosterButton.set_uses_left()`/`set_selected()`/`set_disabled_state()` calls) — a button stays clickable even when disabled (so `game_screen.gd`'s "already used"/"no boosters left" messaging still fires on press), matching the old `BoosterButton` behavior exactly. `BoosterTextureButton` also gained its own `play_feedback()` (a pulse scale/color tween on press, mirroring the old `BoosterButton.play_feedback()`), so `play_booster_feedback()`'s visual feedback is preserved.
- **The old `BoosterButton`/`BoosterButton.tscn` are left in place, unused.** `booster_panel.gd` no longer instantiates them, but the files themselves are untouched (some existing tests still reference them) — no gameplay/UI path reads them anymore.
- **Unchanged by design.** `BoardView`, tile selection/swaps/valid-invalid moves/cascades/damage/HP changes, victory/defeat, no-move shuffle, ice, holes, deterministic levels, `EnemyPanel`'s Stage 64.1 background/sprite/HP-bar presentation and damage feedback wiring, `RoundModifierPanel`'s description-only display, global booster inventory/spending, economy, rewards, `MainMenuScreen`, `LevelSelectScreen`, `SettingsScreen`, `ShopScreen`, and the victory/defeat/result flow are all untouched.
- **No automated tests were added, updated, touched, or run.** `booster_panel_test.gd` (which looks up `Content/ButtonRow/HammerButton` typed as `BoosterButton`) and `booster_button_asset_stub_test.gd` reference the pre-Stage-64.2 structure and will need a future update; manual verification in the Godot editor is expected for this stage.

## Stage 64.3: Game UI Editor-Visible Asset Stabilization v0.1

Stage 64.3 stabilizes the Game UI asset workflow introduced in Stage 64.1/64.2: `BoosterPanel`'s three booster buttons become fixed, editor-visible scene nodes instead of runtime-instantiated controls, and a `BoosterPanel` background asset key/path is added.

- **`BoosterPanel.tscn` gained fixed booster button nodes.** `HammerButton`, `FreezeTimeButton`, and `RocketBarrageButton` (each a `BoosterTextureButton` instance) are now direct children of `ButtonRow`, hand-positioned/resizable in the editor like any other manually-placed control, each with its `booster_id` (`"hammer"`/`"freeze_time"`/`"rocket_barrage"`) set directly in the scene. `PanelBackground` stays the first child (behind `ButtonRow`, `mouse_filter = 2` so it never intercepts clicks).
- **Runtime button creation removed.** `booster_panel.gd` no longer preloads `BoosterTextureButton.tscn` or calls `instantiate()`/clears `ButtonRow`'s children. `_buttons` is populated once in `_ready()` from the three fixed `@onready` node references (`hammer_button`/`freeze_time_button`/`rocket_barrage_button`) via a new `_register_fixed_buttons()`. `setup_boosters(catalog)` still accepts the booster catalog (used only to fill each button's tooltip text via a new `_apply_tooltips()`) but creates no UI nodes.
- **Fixed buttons wired to `booster_pressed` once, safely.** `_register_fixed_buttons()` connects each button's `pressed` signal (guarded by `is_connected()` so it can't double-connect) to `_on_button_pressed.bind(booster_id)`, so pressing `HammerButton`/`FreezeTimeButton`/`RocketBarrageButton` emits `booster_pressed("hammer")`/`("freeze_time")`/`("rocket_barrage")` exactly as before.
- **Booster icon binding is fallback-only.** The new `_bind_booster_icon(button, booster_id)` only assigns `button.default_texture` from `AssetKeyResolver.get_shop_booster_icon_asset_key()`/`GameAssetCatalog` when the button's `default_texture` is still `null` — an Inspector-assigned icon on any of the three fixed buttons is never overwritten. `selected_texture`/`disabled_texture` are never touched by script at all, so they stay fully optional/Inspector-only; missing ones simply fall through to `BoosterTextureButton`'s existing `StateFilter`/`Fallback` state indication.
- **`refresh()`/`play_booster_feedback()`/`get_button_count()` unchanged in behavior**, just reading from the fixed `_buttons` dictionary instead of a dynamically built one; count/selected/disabled visual rules are identical to Stage 64.2.
- **New `BoosterPanel` background asset key.** `AssetKeyResolver.get_ui_asset_key("booster_panel_background")` → `ui_booster_panel_background` → `res://assets/images/ui/game/booster_panel/background.png`, added to `GameAssetCatalog`. A new `_bind_panel_background()` assigns it to `PanelBackground` only when the Inspector hasn't already assigned a texture (`FallbackImageSlot.has_texture()` check), mirroring `EnemyPanel`'s manual-override-safe pattern. The new `assets/images/ui/game/booster_panel/` folder was created with a `.gitkeep` placeholder since the PNG doesn't exist yet.
- **`EnemyPanel` reviewed, unchanged.** `BackgroundVisual`/`EnemySprite` still show their assigned texture or a colored fallback, still preserve any Inspector-assigned texture via their existing manual-override flags, and the HP bar/label behavior from Stage 64.1 is untouched.
- **Unchanged by design.** `GameScreen`'s `HudRow`/`EnemyPanel`/`RoundModifierPanel`/`BoardArea` order, hidden `StatusLabel`, `BoosterPanel`'s expanded height, `HeroPartyPanel` feature-flag visibility, booster selection/spending/global inventory/targeted booster flow, board input/match resolution/damage/victory/defeat/result overlay, economy, rewards, deterministic levels, holes, ice, no-move shuffle, `MainMenuScreen`, `LevelSelectScreen`, `SettingsScreen`, and `ShopScreen` are all untouched.
- **No automated tests were added, updated, touched, or run.** `booster_panel_test.gd` and `booster_button_asset_stub_test.gd` (already stale since Stage 64.2) remain untouched; manual verification in the Godot editor is expected for this stage.

## Stage 64.4: GameScreen Vertical Combat Layout Rework v0.1

Stage 64.4 reworks the `GameScreen` vertical layout: the Level/Moves/Levels row moves from the top down near the booster panel, `EnemyPanel` expands to use more of the upper screen, `RoundModifierPanel` grows and switches to short gameplay-oriented text, and the whole battle stack keeps ~20px top/bottom padding with the board staying centered and square.

- **New `BattleRoot` order.** `GameScreen.tscn`'s top-to-bottom order is now `EnemyPanel` → `RoundModifierPanel` → `BoardArea` → `StatusLabel` (hidden) → `HeroPartyPanel` → `BottomHudRow` → `BoosterPanel`. The old top `HudRow` (`BattleHud` + `MenuButton`) was renamed `BottomHudRow` and moved to just above `BoosterPanel`; no script referenced the row's own node name (only the `%BattleHud`/`%MenuButton` scene-unique children), so the rename/move required no script path changes. `BattleHud`'s level/moves display and `MenuButton`'s `back_pressed` flow are unchanged.
- **`SafeMargin` padding reduced to ~20px top/bottom.** `theme_override_constants/margin_top`/`margin_bottom` went from `28` to `20` (left/right stay `28`), so the battle stack sits closer to the screen edges without touching them.
- **`EnemyPanel` expanded.** `game_screen.gd`'s layout methods now size `enemy_panel.custom_minimum_size` to `(width, 200)` portrait (was `132`) / `(width, 120)` landscape (was `92`) — `EnemyPanel.tscn`'s internal structure (`BackgroundVisual`/`EnemySprite`/`HpBarRoot`/`HitEffectLayer`) is unchanged, so the enemy sprite/background simply get more vertical room to fill, and the HP bar stays anchored near the panel's bottom edge exactly as before.
- **`RoundModifierPanel` roughly doubled.** `round_modifier_panel.custom_minimum_size` is now explicitly set to `(width, 48)` portrait / `(width, 40)` landscape (previously unsized, sized only by its label's natural content height). `ModifierDescriptionLabel` gained `size_flags_vertical = 3` (expand/fill) and `vertical_alignment = 1` (center) so it stays vertically centered in the taller panel instead of sticking to the top.
- **Modifier text is now short and gameplay-oriented.** New `LevelBoostFormatter.format_gameplay_label(boost)` replaces the old `boost.description` as the value written to `modifier_description_label.text` in `_on_level_boost_changed()`: `"x2 Damage Red"`/`"x2 Damage Blue"`/etc. for `COLOR_DAMAGE_MULTIPLIER`, `"x2 Damage Match-4 + x3 Damage Match-5"` for `LARGE_MATCH_MULTIPLIER`, and `"+3 Moves"` for `EXTRA_MOVES` (matching the existing `format_label()`'s already-short `"+%d Moves"` wording). `format_label()` itself is untouched and still feeds the compact debug status line via `format_debug_label()`/`format_debug_info_label()`. The null/`is_none()` boost still hides `RoundModifierPanel` exactly as before, with no separate title label.
- **Board stays centered and square.** `BoardArea`/`BoardView` sizing (`PORTRAIT_BOARD_SIZE`/`LANDSCAPE_BOARD_SIZE`) is untouched; `BattleCenter`'s `CenterContainer` still centers the whole `BattleRoot` stack as one unit, the same mechanism that already kept the board visually centered before this stage.
- **`BoosterPanel`/`EnemyPanel` visual behavior otherwise untouched.** Booster button fixed nodes, counts, selected/disabled states, `booster_pressed`, global inventory refresh (Stage 64.2/64.3), and `EnemyPanel`'s background/sprite fallback, normal/damaged states, and HP bar/label behavior (Stage 64.1) are all preserved exactly.
- **`StatusLabel` stays hidden/safe.** Still `visible = false`, still reserves no layout space, and `_set_status()` remains a safe no-visual-effect call for internal/debug feedback.
- **Unchanged by design.** Board input/swaps/cascades/damage/HP updates, victory/defeat/result overlay, no-move shuffle, ice, holes, deterministic levels, boosters, economy, rewards, `LevelSelectScreen`, `MainMenuScreen`, `SettingsScreen`, and `ShopScreen` are all untouched.
- **No automated tests were added, updated, touched, or run.** `game_screen_layout_test.gd` (already stale since Stage 64.1) remains untouched; manual verification in the Godot editor is expected for this stage.

## Stage 64.5: Game Asset Structure Audit and Rename v0.1

Stage 64.5 audits the `GameScreen`-related asset structure introduced in Stage 39/63.x/64.1-64.3 and renames the `EnemyPanel`'s enemy-state and background asset keys to a clean numeric scheme, without touching gameplay ids or any battle/level configuration.

- **Audit result: nothing was deleted.** Every asset path referenced by `GameAssetCatalog`/`AssetKeyResolver`/`EnemyPanel`/`BoosterPanel` was still either a real, actively-used file (`level_select`/`main_menu`/`settings`/`shop`/`shared` art) or an already-documented `missing/placeholder expected` reserved key with no PNG on disk — there were no genuinely orphaned image files to remove.
- **Enemy sprite state keys renamed.** `gatekeeper_normal`/`gatekeeper_damaged` → `enemy_1_normal`/`enemy_1_damaged` in `game_asset_catalog.gd` (`res://assets/images/ui/game/enemies/enemy_1_normal.png`/`enemy_1_damaged.png`). `AssetKeyResolver.ENEMY_STATE_ASSET_KEYS` now maps `"gatekeeper"` (the unchanged gameplay enemy id from `EnemyCatalog`/`EnemyConfig`) to `{"normal": "enemy_1_normal", "damaged": "enemy_1_damaged"}`, so `enemy_panel.gd`'s `set_enemy_state()`/`get_enemy_normal_asset_key()`/`get_enemy_damaged_asset_key()` calls need no changes. Gameplay enemy ids, `EnemyCatalog`, level/battle configs, and HP/damage balance are untouched, per this stage's explicit scope.
- **Enemy panel background keys renamed.** `enemy_panel_bg_01`..`05` → `enemy_background_1`..`5` in `game_asset_catalog.gd` (`res://assets/images/ui/game/enemy_panel/backgrounds/enemy_background_{1-5}.png`); `AssetKeyResolver.ENEMY_PANEL_BACKGROUND_ASSET_KEYS` returns the five new keys in the same order, and `enemy_panel.gd`'s `_pick_random_background_texture()` random-selection/fallback behavior is unchanged.
- **Missing folder created.** `assets/images/ui/icons/boosters/` (referenced by `shop_icon_booster_hammer`/`_freeze_time`/`_rocket_barrage` since Stage 63.3 but never created on disk) now exists with a `.gitkeep` placeholder. Every other folder required by the current `GameAssetCatalog`/`AssetKeyResolver` surface already existed with a `.gitkeep`.
- **Legacy `assets/images/boosters/booster_*.png` paths kept, not removed.** `booster_hammer`/`booster_freeze_time`/`booster_rocket_barrage` are still `BoosterConfig.icon_asset_key`'s values (via `BoosterCatalog._register_boosters()`) and are still read by the legacy visual-only `BoosterButton`/`BoosterButton.tscn` stub and by several tests (`asset_key_resolver_test.gd`, `booster_button_asset_stub_test.gd`, `game_asset_catalog_test.gd`); removing them would break gameplay-adjacent config and tests, so they were kept per this stage's "keep if still referenced" rule. The active `BoosterPanel`/`BoosterTextureButton` UI already uses the separate shared `shop_icon_booster_*` → `assets/images/ui/icons/boosters/*.png` keys and was not changed.
- **`enemy_gatekeeper` (the general `EnemyCatalog` roster image, distinct from the `EnemyPanel` state sprites) was left unrenamed.** It's keyed off the gameplay enemy id `"gatekeeper"` via `AssetKeyResolver.ENEMY_ASSET_KEYS`, and renaming it would mean renaming the gameplay id itself, which this stage's scope explicitly avoids to prevent any cascade into level/battle configs.
- **Tile, generic-enemy, and remaining UI panel asset keys/paths are unchanged.** `tile_*`, `enemy_training_dummy`/`enemy_small_slime`/etc., `ui_board_frame`/`ui_battle_hud_panel`/`ui_round_modifier_panel`/`ui_result_panel`, and `ui_booster_panel_background` (already correctly wired to `res://assets/images/ui/game/booster_panel/background.png` since Stage 64.3) were verified still correct and left untouched.
- **Documentation updated.** `docs/ASSET_MAP.md` and `docs/ASSET_FOLDERS.md` now reflect the new `enemy_N_normal`/`enemy_N_damaged`/`enemy_background_N` keys/paths and document the numeric naming convention for future enemies; `AGENTS.md`/`docs/design/game_design_v0_1.md` gained a matching Stage 64.5 summary.
- **Unchanged by design.** Board gameplay, enemy HP/damage, victory/defeat, rewards, economy, boosters, `LevelSelectScreen`, `MainMenuScreen`, `SettingsScreen`, `ShopScreen`, deterministic levels, holes, ice, and no-move shuffle are all untouched. `EnemyPanel`/`BoosterPanel` fallback-when-missing behavior and Inspector-assigned-texture-is-never-overwritten behavior are preserved exactly, since only dictionary keys/paths changed, not the binding code paths.
- **No automated tests were added, updated, touched, or run.** Existing tests that reference the old `gatekeeper_normal`/`gatekeeper_damaged`/`enemy_panel_bg_0N` key names (e.g. `game_asset_catalog_test.gd`, `asset_key_resolver_test.gd`) were intentionally left untouched per this stage's explicit test-freeze scope and will need a future update to match the new key names; manual verification in the Godot editor is expected for this stage.

## Stage 64.6: Enemy Code Naming Normalization v0.1

Stage 64.6 extends Stage 64.5's numeric asset renaming down into gameplay code: the ten `EnemyCatalog` enemies (`training_dummy`/`small_slime`/`goblin_scout`/`goblin_fighter`/`armored_goblin`/`wild_wolf`/`bandit`/`orc_brute`/`cave_shaman`/`gatekeeper`) are renamed to `enemy_1`..`enemy_10`, in the exact same registration order, with all HP/attack/intent/target values unchanged — naming cleanup only, no balance tuning.

- **`EnemyConfig` factory methods renamed.** `enemy_config.gd`'s `training_dummy()`..`gatekeeper()` are now `enemy_1()`..`enemy_10()`, each still constructing an `EnemyConfig` with the exact same `max_hp`/`attack`/`intent_turns`/`target_lane` values as its predecessor (e.g. `enemy_1()` still returns `(220, 10, 4, 1)`, `enemy_10()` still returns `(820, 38, 2, 1)`); only `enemy_id` (`"enemy_1"`..`"enemy_10"`) and `display_name` (`"Enemy 1"`..`"Enemy 10"`, replacing the old flavor names) changed.
- **`EnemyCatalog` registration renamed, order preserved.** `_register_enemies()` now calls `ENEMY_CONFIG_SCRIPT.enemy_1()` through `.enemy_10()` in the same sequence the old flavor-named calls used, so `_enemy_order`/`get_all_enemies()` iteration order — and therefore the existing `(level_number - 1) % enemies.size()` level-to-enemy rotation in `LevelCatalog` — is unchanged. `get_default_enemy()` now returns `get_enemy("enemy_1")` instead of `get_enemy("training_dummy")`.
- **`LevelCatalog` fallback renamed.** `_get_fallback_enemy_for_level()`'s empty-catalog fallback is now `ENEMY_CONFIG_SCRIPT.enemy_1()` instead of `.training_dummy()`; the rotation logic itself (`enemy_index := (level_number - 1) % enemies.size()`) is untouched.
- **`AssetKeyResolver` enemy mappings renamed.** `ENEMY_STATE_ASSET_KEYS` now has one entry per enemy id, `"enemy_1"`..`"enemy_10"`, each mapping to its own `{"normal": "enemy_N_normal", "damaged": "enemy_N_damaged"}` (previously only `"gatekeeper"` had an entry, aliased to the Stage 64.5 `enemy_1_*` keys). `ENEMY_ASSET_KEYS` (the older, currently-unused-in-production roster-image map) was migrated the same way, `"enemy_1"`..`"enemy_10"` → `"enemy_N_normal"`, rather than removed, since a public `get_enemy_asset_key()` accessor still exists. `get_enemy_normal_asset_key(enemy_id)`/`get_enemy_damaged_asset_key(enemy_id)` signatures and behavior are unchanged; `enemy_panel.gd` needed no code changes since it already looked up textures generically via `enemy.id`.
- **`GameAssetCatalog` enemy entries normalized.** The ten old per-name roster keys (`enemy_training_dummy`, `enemy_small_slime`, ..., `enemy_gatekeeper` → `res://assets/images/enemies/*.png`) were removed — nothing in active code referenced them once `ENEMY_ASSET_KEYS` was migrated to numeric ids — and `enemy_2_normal`/`enemy_2_damaged` through `enemy_10_normal`/`enemy_10_damaged` (→ `res://assets/images/ui/game/enemies/enemy_N_{normal,damaged}.png`) were added alongside the existing Stage 64.5 `enemy_1_normal`/`enemy_1_damaged` entries. `assets/images/enemies/` is now an unused legacy folder, left in place with its `.gitkeep`.
- **Enemy panel backgrounds untouched.** `enemy_background_1`..`5` (Stage 64.5) are unchanged; no active reference to the older `enemy_panel_bg_0N` names was found anywhere.
- **Boosters untouched.** Booster ids (`hammer`/`freeze_time`/`rocket_barrage`), `shop_icon_booster_*` asset keys, booster inventory, `BoosterPanel`/`BoosterTextureButton` UI, and booster spending logic were not touched by this stage.
