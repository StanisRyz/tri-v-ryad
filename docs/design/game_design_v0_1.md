...
- **Roadmap.** Challenge cycle integration/tuning remains a future stage, unchanged from prior ice-stage roadmap notes; see Stage 58 below for the deterministic layout database that shipped first.

## Stage 58: Deterministic 500-Level Layout Database v0.1

Stage 58 is implemented. Levels 1-500 now load a fixed, pre-generated, pre-validated board/ice layout instead of re-rolling procedural generation on every playthrough. The existing procedural holes/ice generators (Stage 54/57 and their follow-ups) are unchanged — they remain the source of truth that built every stored layout offline, and they remain the runtime fallback for any level without a saved layout. No match, swap, gravity/refill, ice damage, holes, booster, victory/defeat, reward, enemy damage, or LevelSelect rule changed.

- **Data model.** `LevelLayout` (`scripts/game/config/level_layout.gd`) is a small typed object: `level_number`, `archetype`, `variant`, `cycle_position`, `board_mask` (String), `ice_mask` (String), `generation_seed`, `generator_version`, `metadata` (Dictionary). Board size is fixed at 9x9 (81 cells), matching `BoardModel.DEFAULT_WIDTH`/`DEFAULT_HEIGHT`.
- **Mask encoding.** `LevelLayoutMaskCodec` (`scripts/game/config/level_layout_mask_codec.gd`) is the only place that converts between the compact string format and the runtime shapes the board core already consumes. `board_mask` is an 81-character, row-major string (`index = y * 9 + x`): `"1"` means active cell, `"0"` means inactive/hole. `ice_mask` uses the same row-major indexing: `"0"` no ice, `"1"` weak (1-layer) ice, `"2"` strong (2-layer) ice — a layout's `ice_mask` is always `"0"` wherever its `board_mask` is `"0"`, since holes and ice are not mixed in this stage. `board_mask_from_string()`/`ice_mask_string_to_frozen_cells()` produce exactly the `Array`-of-rows shape `BoardModel.set_active_mask()` expects and the `Vector2i`/`{"cell","layers"}` shape `BoardModel.apply_frozen_cells()` expects, so no board-core function changed.
- **Database file.** `data/levels/deterministic_level_layouts.json` holds `version`, `board_size`, `generator_version`, and a `levels` array of all 500 entries. Using compact mask strings instead of per-tile dictionaries and a single JSON file instead of 500 scenes/resources keeps the whole database around 420KB.
- **Generation.** `tools/generate_deterministic_levels.gd` (run headless: `godot --headless --script res://tools/generate_deterministic_levels.gd`) builds every level by calling the same `BoardMaskGenerator.generate_holes_mask_with_metadata()` (holes) and `IcePatternGenerator.generate_frozen_cells()` (ice) functions `BoardChallengeGenerator` already used procedurally, using tier-scoped `HoleGenerationRules.for_tier()`/`IceGenerationRules.for_tier()` resolved from each level's `DifficultyBudgetResolver` tier — but seeded with a stable, reproducible `base_seed + level_number` value instead of a battle-time random seed. The existing 5-level archetype/variant cycle (`ChallengeArchetypeResolver`/`IceVariantResolver`) is preserved exactly: `%5==1` normal, `%5==2` ice weak, `%5==3` holes (`variant = "holes_a"`), `%5==4` ice strong, `%5==0` holes (`variant = "holes_b"`) — `holes_a`/`holes_b` are new naming introduced by this stage to distinguish the two holes cycle positions in metadata; they use the same generation rules and are not a new gameplay mechanic. If a holes level's first seed would have fallen back to a full board (no valid candidate found within the generator's own attempt budget), the tool retries with a deterministic seed-stride offset until a real holes mask validates, so every stored holes level actually has holes.
- **Validation.** `LevelLayoutValidator` (`scripts/game/config/level_layout_validator.gd`) checks: exactly 500 levels, unique and continuous from 1 to 500; `board_mask`/`ice_mask` are each exactly 81 characters with only valid characters; normal levels are a full active board with no ice; weak/strong ice levels are a full active board with only the matching ice variant; holes levels have at least one inactive cell and no ice; ice is never on an inactive cell; a holes level's mask is re-validated through the same `BoardMaskValidator`/tier-scoped `HoleGenerationRules` the generator validated it against originally; ice cell counts are checked against `IceGenerationRules`' density range as a warning (not an error, since a hand-tuned layout could intentionally be atypical); and `metadata` carries `archetype`/`variant`/`cycle_position`/`generation_seed`/`layout_source`. Returns `{valid, errors, warnings, total_levels, counts_by_archetype, counts_by_variant}`. The generated database currently validates with 0 errors and 0 warnings.
- **Loader.** `LevelLayoutDatabase` (`scripts/game/config/level_layout_database.gd`) loads the JSON file once at construction and exposes `has_layout(level_number)`/`get_layout(level_number) -> LevelLayout`. A missing file, unreadable file, invalid JSON, or empty `levels` array is handled gracefully — every `has_layout()` call simply returns `false`, so a broken database degrades to full procedural generation rather than blocking battle startup.
- **BoardChallengeGenerator integration.** `BoardChallengeGenerator.generate()` now checks `LevelLayoutDatabase.has_layout(level_number)` first. On a hit, `_generate_from_layout()` decodes the stored layout's `board_mask`/`ice_mask` directly into `GeneratedBoardChallenge.board_mask`/`frozen_cells` — no RNG, no `BoardMaskGenerator`/`IcePatternGenerator` call — and stamps `metadata["layout_source"] = "deterministic_database"` (plus `ice_variant` for ice levels, for debug-label compatibility). On a miss, generation falls through unchanged to the existing Stage 51-57.5 procedural path, so any level outside 1-500, or any level hit by a corrupted/missing database, still produces a valid board exactly as before this stage.
- **Debug labels.** `GeneratedBoardChallenge.get_debug_label()` now leads with the level number and whether the layout came from the database or procedural fallback, e.g. `"Challenge: normal deterministic, level: 1, seed: 9000001, active: 81/81, holes: 0"`, `"Challenge: ice weak deterministic, level: 2, seed: 9000002, active: 81/81, holes: 0, ice_variant: weak, ice: 37, weak: 37, strong: 0"`, or `"Challenge: holes deterministic, level: 3, seed: 9000003, active: 69/81, holes: 12"`; a procedurally-generated fallback level shows `procedural` in the same position instead of `deterministic`.
- **Roadmap.** Procedural generators remain fully available for this tool, for any level outside the 500-level database, and for future modes (e.g. endless/randomized levels). A future stage could extend the database past 500 levels or add hand-authored overrides for specific levels on top of the same `LevelLayout` format.
