...
- No real ice/frozen-cell behavior, blockers, chains, or portals beyond the current Stage 54.2 v0.1 procedural holes generator with shape variety and pass-through gravity (`holes` archetype generates real inactive-cell masks using block/center shape presets, and `GravityResolver` lets tiles fall through inactive gaps; `normal`/`ice` remain full 9x9) unless explicitly requested.
- No inactive-cell visual polish or pass-through fall animation until Stage 55 is explicitly requested; current rendering only relies on existing safe default/fallback lookups.
